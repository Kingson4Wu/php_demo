 ...
 *
 *      $Id: install_function.php 10737 2010-05-14 06:49:39Z zhangguosheng $
 */
...
function install_testdata($username, $uid) {
	...
	foreach ($arr as $v) {
		if ($v['primaltplname'] == 'portal/portal_topic_content') {
			...
			$setarr['title'] = lang('topic').$topicid;
			$setarr['name'] = 'topic'.$topicid;
			$setarr['useheader'] = 0;
			$setarr['usefooter'] = 1;
			$setarr['uid'] = $uid;
			$setarr['username'] = $username;
			$setarr['dateline'] = time();
			$sql = implode_field_value($setarr);
			$_G['db']->query("INSERT INTO ".$_G['tablepre']."portal_topic SET $sql");
		}
