 ...
 *
 *      $Id: function_seccode.php 10718 2010-05-14 04:46:43Z monkey $
 */
...
function make_seccode($idhash){
	...
	dsetcookie('seccode'.$idhash, authcode(strtoupper($seccode)."\t".(TIMESTAMP - 180)."\t".$idhash."\t".$_G['sid'], 'ENCODE', $_G['config']['security']['authkey']), 360, 1, true);
	return $seccode;
}
...
function make_secqaa($idhash) {
	...
	dsetcookie('secqaa'.$idhash, authcode($_G['cache']['secqaa'][$secqaakey]['answer']."\t".(TIMESTAMP - 180)."\t".$idhash."\t".$_G['sid'], 'ENCODE', $_G['config']['security']['authkey']), 360, 1, true);
