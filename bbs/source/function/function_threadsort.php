...
function gettypetemplate($option, $optionvalue) {
	...
	if(in_array($option['type'], array('number', 'text', 'email', 'calendar', 'image', 'url', 'range', 'upload', 'range'))) {
		if($option['type'] == 'calendar') {
			...
			$showoption[$option['identifier']]['value'] = '<button type="button" class="pn" onclick="uploadWindow(function (aid, url){updatesortattach(aid, url, \''.$_G['setting']['attachurl'].'forum\')})"><span>'.($optionvalue['value'] ? lang('forum/template', 'update') : lang('forum/template', 'upload')).'</span></button>
			...
			if($option['search']) {
				...
				if(in_array($option['type'], array('radio', 'select'))) {
					...
					foreach($option['searchtxt'] as $choice) {
						$value = "$prevs|$choice";
						if($choice) {
							$quicksearch[$optionid]['choices'][$value] = $prevs == 'd' ? $choice.$option['unit'].lang('forum/template', 'lower') : $prevs.'-'.$choice.$option['unit'];
							$prevs = $choice;
						}
						$max = $choice;
					}
					$value = "u|$choice";
					$quicksearch[$optionid]['choices'][$value] .= $max.$option['unit'].lang('forum/template', 'higher');
				}
			}
		}
	}
...
function sortsearch($sortid, $sortoptionarray, $searchoption = array(), $selecturladd = array()) {
	...
	$query = DB::query("SELECT tid FROM ".DB::table('forum_optionvalue')."$sortid ".($selectsql ? 'WHERE '.$selectsql : '')."");
...
function threadsortshow($sortid, $tid) {
	...
	$optiondata = $searchtitle = $searchvalue = $searchunit = $memberinfofield = $_G['forum_option'] = array();
	...
	if($sortoptionarray) {
				...
				if(($option['protect']['usergroup'] && strstr("\t".$option['protect']['usergroup']."\t", "\t$_G[groupid]\t")) || empty($option['protect']['usergroup']) || $_G['forum_thread']['authorid'] == $_G['uid']) {
					if($option['type'] == 'checkbox') {
						...
						if($option['protect']['status'] && $optiondata[$optionid]['value']) {
							$optiondata[$optionid]['value'] = $option['protect']['mode'] == 1 ? '<image src="forum.php?mod=misc&action=protectsort&tid='.$tid.'&sortvalue='.$optiondata[$optionid]['value'].'">' : '<span id="sortmessage_'.$option['identifier'].'"><a href="###" onclick="ajaxget(\'forum.php?mod=misc&action=protectsort&tid='.$tid.'&optionid='.$optionid.'\', \'sortmessage_'.$option['identifier'].'\')">'.lang('forum/template', 'click_view').'</a></span>';
						}

						$_G['forum_option'][$option['identifier']]['value'] = $optiondata[$optionid]['value'] ? $optiondata[$optionid]['value'] : $option['defaultvalue'];
					}
				} else {
					$_G['forum_option'][$option['identifier']]['value'] = lang('forum/template', 'view_noperm');
				}
			} else {
				$_G['forum_option'][$option['identifier']]['value'] = lang('forum/template', 'has_expired');
			}
		}
		...
		if($templatearray['viewthread']) {
			...
			$threadexpiration = $sortdataexpiration ? dgmdate($sortdataexpiration) : lang('forum/template', 'never_expired');
