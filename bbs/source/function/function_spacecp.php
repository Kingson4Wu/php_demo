 ...
 *
 *      $Id: function_spacecp.php 10638 2010-05-13 06:11:56Z zhengqingpeng $
 */
...
function album_update_pic($albumid, $picid=0) {
	...
	if($picid) {
		...
		$piccount = getcount('home_pic', array('albumid'=>$albumid));
		if(empty($piccount)) {
	...
	if($pic['remote']) {
		$picsource = pic_get($pic['filepath'], 'album', $pic['thumb'], $pic['remote'], 0);
	} else {
		$picsource = $basedir.'./album/'.$pic['filepath'];
	}
...
function emailcheck_send($uid, $email) {
	...
	if($uid && $email) {
		...
		$url = $_G['siteurl'].'home.php?mod=misc&ac=emailcheck&amp;hash='.urlencode($hash);
		$mailsubject = lang('spacecp', 'active_email_subject');
		$mailmessage = lang('spacecp', 'active_email_msg', array('url'=>$url));
