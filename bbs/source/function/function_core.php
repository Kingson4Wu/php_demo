 ...
 *
 *      $Id: function_core.php 10986 2010-05-19 05:41:21Z monkey $
 */
...
function rewritedata() {
	...
	if(!defined('IN_ADMINCP')) {
		...
		if(in_array('portal_article', $_G['setting']['rewritestatus'])) {
			$data['search']['portal_article'] = "/".$_G['domain']['pregxp']['portal']."\?mod\=view&(amp;)?aid\=(\d+)?\"([^\>]*)\>/e";
			$data['replace']['portal_article'] = "rewriteoutput('portal_article', 0, '\\1', '\\3', '\\4')";
		}
		...
		if(in_array('all_script', $_G['setting']['rewritestatus'])) {
			$data['search']['all_script'] = "/href\=\"()([a-z]+)\.php\?mod=([^\"]+?)\"([^\>]*)?\>/e";
		...
		$data['rulesearch']['portal_article'] = 'article-{id}.html';
		$data['rulereplace']['portal_article'] = 'portal.php?mod=view&aid={id}';
...
function rewriteoutput($type, $returntype, $host) {
	...
	if($type == 'forum_forumdisplay') {
		...
		if(preg_match("/^http:\/\//is", $url)) {
			...
			if(strpos($url, 'member.php') === 0 || strpos($url, '?mod=ajax') || strpos($url, 'javascript:;') === 0 || $url{0} == '#') {
				$host = '';
			}
			if(!$returntype) {
...
function updatemembercount($uids, $dataarr = array(), $checkgroup = true, $operation = '', $relatedid = 0) {
	...
	if($data) {
		include_once libfile('class/credit');
		$credit = & credit::instance();
		$credit->updatemembercount($data, $uids, $checkgroup);
	}
}
...
function check_seccode($value, $idhash) {
	...
	list($checkvalue, $checktime, $checkidhash, $checksid) = explode("\t", authcode($_G['cookie']['seccode'.$idhash], 'DECODE', $_G['config']['security']['authkey']));
	return $checkvalue == strtoupper($value) && TIMESTAMP - 180 > $checktime && $checkidhash == $idhash && $checksid == $_G['sid'];
}
...
function check_secqaa($value, $idhash) {
	...
	loadcache('secqaa');
	list($checkvalue, $checktime, $checkidhash, $checksid) = explode("\t", authcode($_G['cookie']['secqaa'.$idhash], 'DECODE', $_G['config']['security']['authkey']));
	return $checkvalue == md5($value) && TIMESTAMP - 180 > $checktime && $checkidhash == $idhash && $checksid == $_G['sid'];
}
...
function upload_icon_banner(&$data, $file, $type) {
	...
	if($data['status'] == 3 && $_G['setting']['group_imgsizelimit']) {
		$file['size'] > ($_G['setting']['group_imgsizelimit'] * 1024) && showmessage('file_size_limit', array('size' => $_G['setting']['group_imgsizelimit']));
	}
...
function g_icon($groupid, $return = 0) {
	...
	if(empty($_G['cache']['usergroups'][$groupid]['icon'])) {
		...
		if(substr($_G['cache']['usergroups'][$groupid]['icon'], 0, 5) == 'http:') {
			...
			$s = '<img src="data/attachment/common/'.$_G['cache']['usergroups'][$groupid]['icon'].'" align="absmiddle">';
		}
	}
	if($return) {
		return $s;
