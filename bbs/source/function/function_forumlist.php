 ...
 *
 *      $Id: function_forumlist.php 7610 2010-04-09 01:55:40Z liulanbo $
 */
	...
	if($type == 'selectbox') {
		if($moderators) {
			...
			foreach(explode("\t", $moderators) as $moderator) {
				$modlist .= '<li><a href="home.php?mod=space&username='.rawurlencode($moderator).'">'.($inherit ? '<strong>'.$moderator.'</strong>' : $moderator).'</a></li>';
			}
		} else {
			$modlist = '';
		}
		...
		if($moderators) {
			$modlist = $comma = '';
			foreach(explode("\t", $moderators) as $moderator) {
				$modlist .= $comma.'<a class="notabs" href="home.php?mod=space&username='.rawurlencode($moderator).'">'.($inherit ? '<strong>'.$moderator.'</strong>' : $moderator).'</a>';
				$comma = ', ';
			}
		} else {
			$modlist = '';
