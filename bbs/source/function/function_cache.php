 ...
 *
 *      $Id: function_cache.php 10631 2010-05-13 05:07:18Z tiger $
 */
...
function updatecache($cachename = '') {
	...
	static $cachelist = array('setting','forums','stamps','usergroups','heats',
		...
		'cplugin','grouptype', 'profilesetting', 'userapp', 'creditrule', 'click', 'blockclass',
...
function getcachearray($cachename, $script = '') {
	...
	switch($cachename) {
		...
		case 'fields_required':
			...
			$cols = 'fieldid, invisible, title, description, required, unchangeable, choices';
		...
		case 'userapp':
			$table = 'common_myapp';
			$flag = $cachename == 'myapp' ? 'flag!=\'0\'' : 'flag=\'1\'';
			...
			while($setting = DB::fetch($query)) {
				if($setting['skey'] == 'extcredits') {
					...
					if(!checkformulacredits($setting['svalue'])) {
						...
						$setting['svalue'] = preg_replace("/(digestposts|posts|threads|extcredits[1-8])/", "\$member['\\1']", $setting['svalue']);
					}
				...
				} elseif(in_array($setting['skey'], array('memory', 'search', 'creditspolicy', 'ftp', 'secqaa', 'ec_credit', 'qihoo', 'spacedata', 'infosidestatus', 'uc', 'indexhot', 'outextcredits', 'relatedtag', 'sitemessage', 'uchome', 'heatthread', 'recommendthread', 'disallowfloat', 'allowviewuserthread', 'advtype', 'click', 'rewritestatus', 'rewriterule', 'privacy', 'focus', 'forumkeys'))) {
