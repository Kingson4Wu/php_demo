 ...
 *
 *      $Id: function_magic.php 8544 2010-04-21 05:25:03Z monkey $
 */
...
function magicthreadmod($tid) {
	$query = DB::query("SELECT * FROM ".DB::table('forum_threadmod')." WHERE magicid='0' AND tid='$tid'");
...
function updatemagiclog($magicid, $action, $amount, $price, $targetuid = 0, $idtype = '', $targetid = 0) {
	global $_G;
	DB::query("INSERT INTO ".DB::table('common_magiclog')." (uid, magicid, action, dateline, amount, price, idtype, targetid, targetuid)
		VALUES ('$_G[uid]', '$magicid', '$action', '$_G[timestamp]', '$amount', '$price','$idtype', '$targetid', '$targetuid')", 'UNBUFFERED');
}
...
function magic_check_idtype($id, $idtype) {
	...
	if($tablename) {
		$query = DB::query('SELECT * FROM '.DB::table($tablename)." WHERE $idtype='$id' AND uid = '$_G[uid]'");
...
function magic_peroid($magic, $uid) {
	...
	if($magic['useperoid']) {
		...
		$num = DB::result_first("SELECT count(*) FROM ".DB::table('common_magiclog')." WHERE action='2' AND magicid='$magic[magicid]' AND uid='$uid' AND dateline>'$dateline'");
