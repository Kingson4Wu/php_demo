 ...
 *
 *      $Id: function_home.php 10915 2010-05-18 04:48:02Z monkey $
 */
...
function dstrtotime($string) {
	global $_G;
	$time = '';
	if($string) {
		$time = strtotime($string);
		if(gmdate('H:i', $_G['timestamp'] + $_G['setting']['timeoffset'] * 3600) != date('H:i', $_G['timestamp'])) {
			$time = $time - $_G['setting']['timeoffset'] * 3600;
		}
	}
	return $time;
}
...
function ckprivacy($key, $privace_type) {
	...
	if($privace_type == 'feed') {
		...
		if(!$result && $space['privacy'][$privace_type][$key] == 1) {
...
function space_domain($space) {
	...
	if($_G['setting']['allowdomain'] && $_G['setting']['domainroot']) {
		space_merge($space, 'field_home');
		if($space['domain']) {
			$space['domainurl'] = 'http://'.$space['domain'].'.'.$_G['setting']['domainroot'];
		}
	}
...
function my_checkupdate() {
	global $_G;
	if($_G['setting']['my_app_status'] && empty($_G['setting']['my_closecheckupdate']) && checkperm('admin')) {
...
function pic_cover_get($pic, $picflag) {
	...
	if($picflag == 1) {
		$url = $_G['setting']['attachurl'].'./album/'.$pic;
	} elseif ($picflag == 2) {
		$url = $_G['setting']['ftp']['attachurl'].'./album/'.$pic;
	} else {
		$url = $pic;
	}
	return $url;
...
function member_status_update($uid, $counts) {
	...
	foreach ($counts as $key => $value) {
		...
		if(in_array($key, array('notifications','pokes','groupinvitations','activityinvitations','myinvitations','pendingfriends'))) {
	...
	if($_G['uid'] && $_G['setting']['my_app_status']) {
		...
		while($value = DB::fetch($query)) {
			...
			if($value['allowsidenav'] && !isset($_G['cache']['userapp'][$value['appid']])) {
				if($_G['member']['menunum'] < 5) $_G['member']['menunum'] = 10;
