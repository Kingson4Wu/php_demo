 ...
 *
 *      $Id: spacecp_eccredit.php 7434 2010-04-07 07:33:51Z monkey $
 */
...
if($_G['gp_op'] == 'list') {
	...
	switch($filter) {
		case 'thisweek':
			$sql .= " AND dateline>=$_G[timestamp] - 604800";
			break;
		case 'thismonth':
			$sql .= " AND dateline>=$_G[timestamp] - 2592000";
			break;
		case 'halfyear':
			$sql .= " AND dateline>=$_G[timestamp] - 15552000";
			break;
		case 'before':
			$sql .= " AND dateline<$_G[timestamp] - 15552000";
			break;
	...
	$num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('forum_tradecomment')." WHERE $sql");
	...
	$comments = array();
	$query = DB::query("SELECT tc.*, tl.subject, tl.price, tl.credit FROM ".DB::table('forum_tradecomment')." tc LEFT JOIN ".DB::table('forum_tradelog')." tl ON tl.orderid=tc.orderid WHERE $sql ORDER BY dateline DESC LIMIT $start_limit, 10");
