 ...
 *
 *      $Id: spacecp_poke.php 10589 2010-05-12 10:21:17Z zhengqingpeng $
 */
...
if($op == 'send' || $op == 'reply') {
	...
	if(submitcheck('pokesubmit')) {
		...
		$setarr = array(
			...
			'uid' => $uid,
			'fromuid' => $_G['uid'],
			'note' => getstr($_POST['note'], 150, 1, 1, 1),
		...
		showmessage('poke_success', dreferer(), array('username' => $tospace['username'], 'uid' => $uid), array('showdialog'=>1, 'showmsg' => true, 'closetime' => 1));
	...
	showmessage('has_been_hailed_overlooked', '', array('uid' => $uid), array('showdialog'=>1, 'showmsg' => true, 'closetime' => 1));
