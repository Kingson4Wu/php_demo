 ...
 *
 *      $Id: thread_poll.php 6752 2010-03-25 08:47:54Z cnteacher $
 */
...
if($count = $sdb->fetch_first("SELECT MAX(votes) AS max, SUM(votes) AS total FROM ".DB::table('forum_polloption')." WHERE tid='$_G[tid]'")) {


	$options = $sdb->fetch_first("SELECT * FROM ".DB::table('forum_poll')." WHERE tid='$_G[tid]'");
	...
	$query = $sdb->query("SELECT polloptionid, votes, polloption, voterids FROM ".DB::table('forum_polloption')." WHERE tid='$_G[tid]' ORDER BY displayorder");
	...
	$allowvotethread = (!$_G['forum_thread']['closed'] && !checkautoclose($_G['forum_thread']) || $_G['group']['alloweditpoll']) && TIMESTAMP < $expirations && $expirations > 0;
