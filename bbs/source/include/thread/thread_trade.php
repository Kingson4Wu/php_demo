 ...
 *
 *      $Id: thread_trade.php 9717 2010-05-04 10:16:14Z liguode $
 */
...
if(empty($_G['gp_do']) || $_G['gp_do'] == 'tradeinfo') {
	...
	if($_G['gp_do'] == 'tradeinfo') {
		...
		m.credits, m.email, mp.gender, mp.site,	mp.icq, mp.qq, mp.yahoo, mp.msn, mp.taobao, mp.alipay, mf.medals,
		mf.sightml AS signature, mf.customstatus, ms.buyercredit, ms.sellercredit, mc.* $fieldsadd
		...
		if(!$_G['inajax']) {
			...
			$limit = 10;
			$query = DB::query("SELECT tid, subject, dateline FROM ".DB::table('forum_thread')." WHERE authorid='".$_G['forum_thread']['authorid']."' AND special='2' ORDER BY dateline DESC LIMIT $limit");
			while($userthread = DB::fetch($query)) {
