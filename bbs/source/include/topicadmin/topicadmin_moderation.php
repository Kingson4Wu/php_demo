 ...
 *
 *      $Id: topicadmin_moderation.php 10941 2010-05-18 08:12:38Z wangjinbo $
 */
			...
			if($operation == 'stick') {
				...
				foreach($threadlist as $thread) {
					if($thread['digest'] != $digestlevel) {
						...
						updatecreditbyaction('digest', $thread['authorid'], $extrasql, '', $digestlevel - $thread['digest']);
					}
				}
				...
				if($isrecommend) {
					...
					foreach($threadlist as $thread) {
						...
						if($selectattach) {
							...
							$filename = 'forum.php?mod=image&aid='.$selectattach.'&size='.$imgw.'x'.$imgh.'&key='.rawurlencode($key);
						} else {
							$selectattach = 0;
							$filename = '';
						}
