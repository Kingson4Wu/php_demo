 ...
 *
 *      $Id: post_editpost.php 10573 2010-05-12 08:41:22Z monkey $
 */
...
} elseif((!$_G['forum']['ismoderator'] || !$_G['group']['alloweditpost'] || (in_array($orig['adminid'], array(1, 2, 3)) && $_G['adminid'] > $orig['adminid'])) && !($_G['forum']['alloweditpost'] && $isorigauthor)) {
...
if(!submitcheck('editsubmit')) {
	...
	$postinfo['message'] = preg_replace($postinfo['htmlon'] ? $language['post_edithtml_regexp'] : $language['post_edit_regexp'], '', $postinfo['message']);
	...
	if(empty($_G['gp_delete'])) {
		...
		if($isfirstpost) {
			...
			if($thread['special'] == 3) {
				$price = $thread['price'] > 0 && $thread['price'] != $_G['gp_rewardprice'] ? $_G['gp_rewardprice'] : 0;
			}
			...
			if($thread['special'] == 1 && ($_G['group']['alloweditpoll'] || $isorigauthor) && !empty($_G['gp_polls'])) {
				...
				if($pollarray['options']) {
					if(count($pollarray['options']) > 20) {
						showmessage('post_poll_option_toomany', '', array('maxpolloptions' => 20));
					}
				...
				DB::query("UPDATE ".DB::table('forum_activity')." SET cost='$activity[cost]', starttimefrom='$activity[starttimefrom]', starttimeto='$activity[starttimeto]', place='$activity[place]', class='$activity[class]', gender='$activity[gender]', number='$activity[number]', expiration='$activity[expiration]' WHERE tid='$_G[tid]'", 'UNBUFFERED');
			...
			DB::query("UPDATE ".DB::table('forum_thread')." SET typeid='$typeid', sortid='$sortid', subject='$subject', readperm='$readperm', price='$price' $authoradd $polladd ".($_G['forum_auditstatuson'] && $audit == 1 ? ",displayorder='0', moderated='1'" : '').", status='$thread[status]' WHERE tid='$_G[tid]'", 'UNBUFFERED');
		...
		if($_G['setting']['editedby'] && (TIMESTAMP - $orig['dateline']) > 60 && $_G['adminid'] != 1) {
			...
			$message = lang('forum/misc', $htmlon ? 'post_edithtml' : 'post_edit', array('editor' => $editor, 'edittime' => $edittime)) . $message;
		}
		...
		if($thread['special'] == 3 && $isfirstpost) {
			...
			DB::delete('common_credit_log', array('uid' => $thread['authorid'], 'operation' => 'RTC', 'realtedid' => $_G['tid']));
		}
	...
	if($_G['forum_auditstatuson']) {
		...
		if(!empty($_G['gp_delete']) && $isfirstpost) {
			...
			if($isfirstpost && $modnewthreads) {
				my_thread_log('delete', array('tid' => $_G['tid']));
				showmessage('edit_newthread_mod_succeed', "forum.php?mod=forumdisplay&fid=$_G[fid]", $param);
			} elseif(!$isfirstpost && $modnewreplies) {
				my_post_log('delete', array('pid' => $pid));
