 ...
 *
 *      $Id: modcp_moderate.php 10471 2010-05-11 09:14:09Z monkey $
 */
...
if($op == 'members') {
	...
	if(submitcheck('dosubmit', 1) || submitcheck('modsubmit')) {
		...
		if(empty($_G['gp_modact']) || !in_array($_G['gp_modact'], array('invalidate', 'validate', 'delete'))) {
		...
		if(submitcheck('dosubmit', 1)) {
			...
			showmessage('modcp_mod_succeed', "{$cpscript}?action=$_G[gp_action]&op=$op&filter=$filter");
		...
		$multipage = multi(DB::result($query, 0), $_G['setting']['memberperpage'], $page, "{$cpscript}?action=$_G[gp_action]&op=$op&fid=$_G[fid]&filter=$filter");
...
if($op == 'replies') {
	...
	if($modcount) {
		$postarray = getallwithposts(array(
			'select' => 'f.name AS forumname, f.allowsmilies, f.allowhtml, f.allowbbcode, f.allowimgcode, p.pid, p.fid, p.tid, p.authorid, p.authorid, p.subject, p.dateline, p.message, p.useip, p.attachment, p.htmlon, p.smileyoff, p.bbcodeoff, t.subject AS tsubject',
	...
	$multipage = multi($modcount, $_G['tpp'], $page, "{$cpscript}?action=$_G[gp_action]&op=$op&filter=$filter&fid=$_G[fid]");
	...
	if($modcount) {
		...
		$query = DB::query("SELECT t.tid, t.fid, t.posttableid, t.author, t.sortid, t.authorid, t.subject as tsubject, t.dateline, t.attachment
			...
			ORDER BY t.dateline DESC LIMIT $start_limit, $_G[tpp]");
