 ...
 *
 *      $Id: modcp_recyclebin.php 10216 2010-05-07 09:00:23Z zhaoxiongfei $
 */
...
if($_G['fid'] && $_G['forum']['ismoderator'] && $modforums['recyclebins'][$_G['fid']]) {
	...
	if($op == 'search' &&  submitcheck('searchsubmit')) {
		...
		if($sql) {
			...
			$query = DB::query("SELECT tid FROM ".DB::table('forum_thread')." WHERE fid='$_G[fid]' AND displayorder='-1' $sql ORDER BY displayorder DESC, lastpost DESC LIMIT 1000");
			$tids = $comma = '';
			$count = 0;
			while($tid = DB::fetch($query)) {
				$tids .= $comma.$tid['tid'];
	...
	if($op == 'list') {
		...
		if($total) {
			$start = ($page - 1) * $_G['tpp'];
			$query = DB::query("SELECT * FROM ".DB::table('forum_thread')." WHERE fid='$_G[fid]' AND displayorder='-1' ORDER BY displayorder DESC, lastpost DESC LIMIT $start, $_G[tpp]");
		}
	}
