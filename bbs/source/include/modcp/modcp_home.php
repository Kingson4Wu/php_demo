 ...
 *
 *      $Id: modcp_home.php 6752 2010-03-25 08:47:54Z cnteacher $
 */
...
while($note = DB::fetch($query)) {
	if($note['expiration'] < TIMESTAMP) {
		...
		$note['checkbox'] = '<input class="checkbox" type="checkbox" name="delete[]" '.($note['admin'] == $_G['member']['username'] || $_G['adminid'] == 1 ? "value=\"$note[id]\"" : 'disabled').'>';
