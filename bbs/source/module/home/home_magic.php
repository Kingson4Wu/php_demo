...
/*
	...
	$Id: home_magic.php 8587 2010-04-21 07:53:59Z monkey $
*/
...
} elseif(!$_G['setting']['magicstatus'] && $_G['adminid'] != 1) {
...
if(empty($action) && !empty($_G['gp_mid'])) {
	if($_G['gp_magicid'] = DB::result_first("SELECT m.magicid FROM ".DB::table('common_member_magic')." mm INNER JOIN ".DB::table('common_magic')." m ON mm.magicid=m.magicid WHERE mm.uid='$_G[uid]' AND m.identifier='$_G[gp_mid]'")) {
		$action = 'mybox';
		$operation = 'use';
	} else {
		$action = 'shop';
...
if($action == 'shop') {
	...
	if(in_array($operation, array('index', 'hot'))) {
		...
		$query = DB::query("SELECT magicid, name, identifier, description, price, num, salevolume, weight FROM ".DB::table('common_magic')." WHERE available='1' $filteradd $orderby LIMIT $start_limit,$_G[tpp]");
		...
		if(!submitcheck('operatesubmit')) {
			...
			if(getuserprofile('extcredits'.$_G['setting']['creditstransextra'][3]) < $totalprice) {
				showmessage('magics_credits_no_enough', '', array('credit' => $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['title']));
			...
			updatemagiclog($magic['magicid'], '1', $magicnum, $magic['price'], $_G['uid']);

			DB::query("UPDATE ".DB::table('common_magic')." SET num=num+(-'$magicnum'), salevolume=salevolume+'$magicnum' WHERE magicid='$magic[magicid]'");
			updatemembercount($_G['uid'], array($_G['setting']['creditstransextra'][3] => -$totalprice));
			showmessage('magics_buy_succeed', 'home.php?mod=magic&action=mybox', array('magicname' => $magic['name'], 'num' => $magicnum, 'credit' => $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['title'].' '.$totalprice.' '.$_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['unit']));
		...
		if(!submitcheck('operatesubmit')) {
			...
			updatemembercount($_G['uid'], array($_G['setting']['creditstransextra'][3] => -$totalprice));
			showmessage('magics_buygive_succeed', 'home.php?mod=magic&action=mybox', array('magicname' => $magic['name'], 'toname' => $toname, 'num' => $magicnum, 'credit' => $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['title'].' '.$totalprice.' '.$_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['unit']));
	...
	if(empty($operation)) {
		...
		if($operation == 'use') {
			...
			if(!submitcheck('operatesubmit')) {
				...
				updatemembercount($_G['uid'], array($_G['setting']['creditstransextra'][3] => $totalprice));
				showmessage('magics_sell_succeed', 'home.php?mod=magic&action=mybox', array('magicname' => $magic['name'], 'num' => $magicnum, 'credit' => $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['title'].' '.$totalprice.' '.$_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['unit']));
			}
			...
			if(!submitcheck('operatesubmit')) {
				...
				showmessage('magics_drop_succeed', 'home.php?mod=magic&action=mybox', array('magicname' => $magic['name'], 'num' => $magicnum));
			}
	...
	$loglist = array();
	if($operation == 'uselog') {
		$query = DB::query("SELECT COUNT(*) FROM ".DB::table('common_magiclog')." WHERE action='2' AND uid='$_G[uid]'");
