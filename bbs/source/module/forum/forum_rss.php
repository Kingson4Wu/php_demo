 ...
 *
 *      $Id: forum_rss.php 10785 2010-05-17 00:56:41Z wangjinbo $
 */
...
if($fidarray) {
	$query = DB::query("SELECT * FROM ".DB::table('forum_rsscache')." WHERE fid IN (".implode(',', $fidarray).") ORDER BY dateline DESC LIMIT $num");
	if(DB::num_rows($query)) {
		while($thread = DB::fetch($query)) {
			if(TIMESTAMP - $thread['lastupdate'] > $ttl * 60) {
				...
				if($attachfile) {
					if($attach['remote']) {
						$filename = $_G['siteurl'].$_G['setting']['ftp']['attachurl'].'forum/'.$attachfile;
					} else {
