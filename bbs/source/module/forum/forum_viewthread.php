 ...
 *
 *      $Id: forum_viewthread.php 10953 2010-05-18 09:12:51Z zhangguosheng $
 */
...
if($_G['setting']['cachethreadlife'] && $_G['forum']['threadcaches'] && !$_G['uid'] && $page == 1 && !$_G['forum']['special'] || 1) {
...
$_G['forum_thread']['subject'] = ($threadsort ? ($_G['forum']['threadsorts']['listable'] ? '<a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'&amp;filter=sort&amp;sortid='.$_G['forum_thread']['sortid'].'">['.$_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']].']</a>' : '['.$_G['forum']['threadsorts']['types'][$_G['forum_thread']['sortid']].']').' ' : '')
	.($_G['forum_thread']['typeid'] && $_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']] ? ($_G['forum']['threadtypes']['listable'] ? '<a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'&amp;filter=type&amp;typeid='.$_G['forum_thread']['typeid'].'">['.$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']].']</a>' : '['.$_G['forum']['threadtypes']['types'][$_G['forum_thread']['typeid']].']').' ' : '')
...
$allowpostreply = ($_G['forum']['allowreply'] != -1) && ((!$_G['forum_thread']['closed'] && !checkautoclose($_G['forum_thread'])) || $_G['forum']['ismoderator']) && ((!$_G['forum']['replyperm'] && $_G['group']['allowreply']) || ($_G['forum']['replyperm'] && forumperm($_G['forum']['replyperm'])) || $_G['forum']['allowreply']);
...
$relatedkeywords = $tradekeywords = $metakeywords = $_G['forum_firstpid'] = '';
...
if(empty($_G['gp_viewpid'])) {
	...
	if($_G['forum_pagebydesc']) {
		...
		if($ordertype != 1) {
			$pageadd = "ORDER BY dateline LIMIT $start_limit, $_G[ppp]";
		} else {
			$_G['forum_numpost'] = $_G['forum_thread']['replies'] + 2 - $_G['forum_numpost'] + ($page > 1 ? 1 : 0);
			$pageadd = "ORDER BY first DESC,dateline DESC LIMIT $start_limit, $_G[ppp]";
		}
	}
...
if($_G['forum_thread']['special'] > 0 && (empty($_G['gp_viewpid']) || $_G['gp_viewpid'] == $_G['forum_firstpid'])) {
	...
	switch($_G['forum_thread']['special']) {
		...
		case 127:
			$sppos = strrpos($postlist[$_G['forum_firstpid']]['message'], chr(0).chr(0).chr(0));
