 ...
 *
 *      $Id: forum_forumdisplay.php 10340 2010-05-10 10:33:14Z zhaoxiongfei $
 */
...
$_G['gp_digest'] = isset($_G['gp_digest']) ? $_G['gp_digest'] : '';
$_G['gp_filter'] = isset($_G['gp_filter']) ? $_G['gp_filter'] : '';
...
if($_G['forum']['type'] == 'forum') {
	if(empty($_G['gp_archiveid'])) {
		$navigation = '&rsaquo; <a href="forum.php">'.$_G['setting']['navs'][2]['navname'].'</a> &rsaquo; '.$_G['forum']['name'];
	} else {
		$navigation = '&rsaquo; <a href="forum.php">'.$_G['setting']['navs'][2]['navname'].'</a> &rsaquo; '.'<a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'">'.$_G['forum']['name'].'</a> &rsaquo; '.$forumarchive[$_G['gp_archiveid']]['displayname'];
	}
	$navtitle = $_G['forum']['name'];
} else {
	$forumup = $_G['forum']['status'] == 3 ? '' : $_G['cache']['forums'][$_G['forum']['fup']]['name'];
	if(empty($_G['gp_archiveid'])) {
		$navigation = '&rsaquo; <a href="forum.php">'.$_G['setting']['navs'][2]['navname'].'</a> &rsaquo; <a href="forum.php?mod=forumdisplay&fid='.$_G['forum']['fup'].'">'.$forumup.'</a> &rsaquo; '.$_G['forum']['name'];
	} else {
		$navigation = '&rsaquo; <a href="forum.php">'.$_G['setting']['navs'][2]['navname'].'</a> &rsaquo; <a href="forum.php?mod=forumdisplay&fid='.$_G['forum']['fup'].'">'.$forumup.'</a> &rsaquo; '.'<a href="forum.php?mod=forumdisplay&fid='.$_G['fid'].'">'.$_G['forum']['name'].'</a> &rsaquo; '.$forumarchive[$_G['gp_archiveid']]['displayname'];
	}
...
if($_G['forum']['status'] == 3) {
	$navtitle = lang('group/template', 'group').' - '.$_G['forum']['name'];
...
foreach($_G['cache']['forums'] as $sub) {
	if($sub['type'] == 'sub' && $sub['fup'] == $_G['fid'] && (!$_G['setting']['hideprivate'] || !$sub['viewperm'] || forumperm($sub['viewperm']) || strstr($sub['users'], "\t$_G[uid]\t"))) {
		...
		$subexists = 1;
		$sublist = array();
		$sql = !empty($_G['member']['accessmasks']) ? "SELECT f.fid, f.fup, f.type, f.name, f.threads, f.posts, f.todayposts, f.lastpost, ff.description, ff.moderators, ff.icon, ff.viewperm, ff.extra, ff.redirect, a.allowview FROM ".DB::table('forum_forum')." f
					...
					: "SELECT f.fid, f.fup, f.type, f.name, f.threads, f.posts, f.todayposts, f.lastpost, ff.description, ff.moderators, ff.icon, ff.viewperm, ff.extra, ff.redirect FROM ".DB::table('forum_forum')." f
...
if($_G['forum']['threadsorts']['types'] && $sortoptionarray && ($_G['gp_searchoption'] || $_G['gp_searchsort'])) {
	...
	if($searchsorttids = sortsearch($_G['gp_sortid'], $sortoptionarray, $_G['gp_searchoption'], $selectadd)) {
...
$multipage = multi($_G['forum_threadcount'], $_G['tpp'], $page, "forum.php?mod=forumdisplay&fid=$_G[fid]".($_G['gp_filter'] ? "&filter=$_G[gp_filter]" : '')."$forumdisplayadd[orderby]$forumdisplayadd[page]$multipage_archive", $_G['setting']['threadmaxpages']);
$extra = rawurlencode('page='.$page.($forumdisplayadd['page'] ? '&filter='.$_G['gp_filter'].$forumdisplayadd['page'] : ''));
...
while(($querysticky && $thread = DB::fetch($querysticky)) || ($query && $thread = DB::fetch($query))) {
	...
	if($topicposts > $_G['ppp']) {
		...
		for($i = 2; $i <= 6 && $i <= $thread['pages']; $i++) {
			$pagelinks .= "<a href=\"forum.php?mod=viewthread&tid=$thread[tid]&amp;".(!empty($multipate_archive) ? "$multipate_archive&amp;" : '')."extra=$extra&amp;page=$i\">$i</a>";
		}
		if($thread['pages'] > 6) {
			$pagelinks .= "..<a href=\"forum.php?mod=viewthread&tid=$thread[tid]&amp;".(!empty($multipate_archive) ? "$multipate_archive&amp;" : '')."extra=$extra&amp;page=$thread[pages]\">$thread[pages]</a>";
		}
...
$addfeedcheck = getuserprofile('publishfeed') & 1 ? 'checked="checked"': '';
