 ...
 *
 *      $Id: forum_topicadmin.php 10921 2010-05-18 06:20:12Z wangjinbo $
 */
...
$_G['inajax'] = 1;
$_G['gp_topiclist'] = !empty($_G['gp_topiclist']) ? array_unique($_G['gp_topiclist']) : array();
...
if($_G['gp_action'] == 'moderate') {
	...
	if(!submitcheck('modsubmit')) {
		...
		if(!DB::result_first("SELECT count(*) FROM ".DB::table('forum_postcomment')." WHERE pid='$pid'")) {
			DB::update('forum_post', array('comment' => 0), "pid='$pid'");
		}
		$posttable = getposttablebytid($_G['tid']);
		$post = DB::fetch_first("SELECT * FROM ".DB::table($posttable)." WHERE pid='$pid'");
		updatepostcredits('-', $post['authorid'], 'reply', $_G['fid']);

		$query = DB::query('SELECT comment FROM '.DB::table('forum_postcomment')." WHERE pid='$pid' AND score='1'");
		$totalcomment = array();
		...
		if($totalv) {
			DB::update('forum_postcomment', array('comment' => $totalv, 'dateline' => TIMESTAMP + 1), "pid='$pid' AND authorid='0'");
		} else {
			DB::delete('forum_postcomment', "pid='$pid' AND authorid='0'");
		}

		$resultarray = array(
		'redirect'	=> "forum.php?mod=viewthread&tid=$_G[tid]&page=$page",
		'reasonpm'	=> ($sendreasonpm ? array('data' => $post, 'var' => 'post', 'item' => 'reason_delete_comment') : array()),
		'reasonvar'	=> array('tid' => $thread['tid'], 'pid' => $post['pid'], 'subject' => $thread['subject'], 'modaction' => $modaction, 'reason' => stripslashes($reason)),
		'modtids'	=> 0,
		'modlog'	=> $thread
		);
	...
	if(!submitcheck('modsubmit')) {
		...
		$extcredit = 'extcredit'.$_G['setting']['creditstransextra'][1];
	...
	$posttable = getposttablebytid($_G['tid']);
	$query = DB::query("SELECT p.pid, p.authorid, p.author, p.status, p.dateline, p.message, m.adminid FROM ".DB::table($posttable)." p LEFT JOIN ".DB::table('common_member')." m ON p.authorid=m.uid WHERE pid IN ($warnpids) AND p.tid='$_G[tid]'");
	while($post = DB::fetch($query)) {
	...
	if(!submitcheck('modsubmit')) {
		...
		'modaction'	=> $_G['gp_stamp'] !== '' ? 'S'.sprintf('%02d', $_G['gp_stamp']) : 'SPD',
		'modlog'	=> $thread
		);
		$modpostsnum = 1;
	...
	foreach($topiclist as $pid) {
		$post = DB::fetch_first("SELECT p.tid, p.authorid, p.dateline, p.first, t.special FROM ".DB::table($posttable)." p LEFT JOIN ".DB::table('forum_thread')." t USING(tid) WHERE p.pid='$pid'");
