...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		$articlelist = array();
		$query = $sdb->query("SELECT at.*,ac.viewnum, ac.commentnum FROM ".DB::table('portal_article_title')." at LEFT JOIN ".DB::table('portal_article_count')." ac ON at.aid=ac.aid WHERE at.aid IN ($index[ids]) ORDER BY $orderby $ascdesc LIMIT $start_limit, $_G[tpp]");
		...
		if($searchindex['id']) {
			...
			if(!$srchtxt) {
				showmessage('search_invalid', 'search.php?mod=portal');
			}

			if($_G['adminid'] != '1' && $_G['setting']['search']['portal']['maxspm']) {
				if(($sdb->result_first("SELECT COUNT(*) FROM ".DB::table('common_searchindex')." WHERE srchmod='$srchmod' AND dateline>'$_G[timestamp]'-60")) >= $_G['setting']['search']['portal']['maxspm']) {
			...
			$query = $sdb->query("SELECT aid FROM ".DB::table('portal_article_title')." WHERE title LIKE '$srchtxtsql%' ORDER BY aid DESC LIMIT ".$_G['setting']['search']['portal']['maxsearchresults']);
			while($article = $sdb->fetch_array($query)) {
				$ids .= ','.$article['aid'];
				$num++;
			}
