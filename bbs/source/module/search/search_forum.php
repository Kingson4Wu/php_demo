 ...
 *
 *      $Id: search_forum.php 10510 2010-05-12 02:08:28Z zhaoxiongfei $
 */
...
if($_G['setting']['my_search_status'] && !$srchfrom && !$searchid) {
	header("Location: search.php?mod=my&q=".urlencode($keyword)."&module=forum".($_G['gp_adv'] ? "&isAdv=1" : ''));
	die;
}
...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		$threadlist = array();
		$query = $sdb->query("SELECT * FROM ".DB::table('forum_thread')." WHERE tid IN ($index[ids]) AND displayorder>='0' ORDER BY $orderby $ascdesc LIMIT $start_limit, $_G[tpp]");
		...
		if($searchindex['id']) {
			...
			if(!$srchtxt && !$srchuid && !$srchuname && !$srchfrom && !in_array($srchfilter, array('digest', 'top')) && !is_array($special)) {
				showmessage('search_invalid', 'search.php?m=forum');
			...
			if($_G['adminid'] != '1' && $_G['setting']['search']['forum']['maxspm']) {
				if(($sdb->result_first("SELECT COUNT(*) FROM ".DB::table('common_searchindex')." WHERE srchmod='$srchmod' AND dateline>'$_G[timestamp]'-60")) >= $_G['setting']['search']['forum']['maxspm']) {
			...
			if($srchtype == 'fulltext' && $_G['setting']['sphinxon']) {
				...
				if(!empty($srchfrom) && empty($srchtxt) && empty($srchuid) && empty($srchuname)) {
					...
					if($srchuname) {
						...
						$query = $sdb->query("SELECT uid FROM ".DB::table('common_member')." WHERE username LIKE '".str_replace('_', '\_', $srchuname)."' LIMIT 50");
						while($member = $sdb->fetch_array($query)) {
							$uids[] = $member['uid'];
						}
						if(count($uids) == 0) {
							$uids = array(0);
				...
				$query = $sdb->query("SELECT ".($srchtype == 'fulltext' ? 'DISTINCT' : '')." t.tid, t.closed, t.author $sqlsrch ORDER BY tid DESC LIMIT ".$_G['setting']['search']['forum']['maxsearchresults']);
				while($thread = $sdb->fetch_array($query)) {
