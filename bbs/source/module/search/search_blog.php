 ...
 *
 *      $Id: search_blog.php 8769 2010-04-22 08:57:30Z redstone $
 */
	...
	if(!empty($searchid)) {
		...
		while ($value = DB::fetch($query)) {
			if(ckfriend($value['uid'], $value['friend'], $value['target_ids'])) {
				if($value['friend'] == 4) {
					...
					$value['message'] = getstr($value['message'], 255, 0, 0, 0, 0, -1);
				}
		...
		if($searchindex['id']) {
			...
			if(!$srchtxt && !$srchuid && !$srchuname) {
				showmessage('search_invalid', 'search.php?mod=blog');
			}

			if($_G['adminid'] != '1' && $_G['setting']['search']['blog']['maxspm']) {
				if(($sdb->result_first("SELECT COUNT(*) FROM ".DB::table('common_searchindex')." WHERE srchmod='$srchmod' AND dateline>'$_G[timestamp]'-60")) >= $_G['setting']['search']['blog']['maxspm']) {
			...
			$query = $sdb->query("SELECT blogid FROM ".DB::table('home_blog')." WHERE subject LIKE '$srchtxtsql%' ORDER BY blogid DESC LIMIT ".$_G['setting']['search']['blog']['maxsearchresults']);
			while($blog = $sdb->fetch_array($query)) {
				$ids .= ','.$blog['blogid'];
				$num++;
			}
