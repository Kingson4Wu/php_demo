 ...
 *
 *      $Id: search_group.php 9843 2010-05-05 05:38:57Z wangjinbo $
 */
...
if(!submitcheck('searchsubmit', 1)) {
	...
	if(!empty($searchid)) {
		...
		if($searchstring[2]) {
			...
			$grouplist = array();
			$query = $sdb->query("SELECT f.*, ff.description, ff.membernum, ff.icon FROM ".DB::table('forum_forum')." f LEFT JOIN ".DB::table('forum_forumfield')." ff ON f.fid=ff.fid WHERE f.fid IN ($index[ids]) AND f.status='3' AND `type`='sub' LIMIT $start_limit, $_G[tpp]");
		...
		if($searchindex['id']) {
			...
			if($srchfid) {
				$query = $sdb->query("SELECT tid FROM ".DB::table('forum_thread')." WHERE fid='$srchfid' AND isgroup='1' AND subject LIKE '%$srchtxtsql%' ORDER BY tid DESC LIMIT ".$_G['setting']['search']['group']['maxsearchresults']);
				while($thread = $sdb->fetch_array($query)) {
					$ids .= ','.$thread['tid'];
					$num++;
				}
				...
				$query = $sdb->query("SELECT fid FROM ".DB::table('forum_forum')." WHERE `type`='sub' AND status='3' AND name LIKE '%$srchtxtsql%' LIMIT ".$_G['setting']['search']['group']['maxsearchresults']);
				while($group = $sdb->fetch_array($query)) {
					$ids .= ','.$group['fid'];
					$num++;
				}
