 ...
 *
 *      $Id: search_album.php 8391 2010-04-20 07:20:44Z redstone $
 */
	...
	if(!empty($searchid)) {
		...
		if($searchindex['id']) {
			...
			if(!$srchtxt && !$srchuid && !$srchuname) {
				showmessage('search_invalid', 'search.php?mod=album');
			}

			if($_G['adminid'] != '1' && $_G['setting']['search']['album']['maxspm']) {
				if(($sdb->result_first("SELECT COUNT(*) FROM ".DB::table('common_searchindex')." WHERE srchmod='$srchmod' AND dateline>'$_G[timestamp]'-60")) >= $_G['setting']['search']['album']['maxspm']) {
			...
			$query = $sdb->query("SELECT albumid FROM ".DB::table('home_album')." WHERE albumname LIKE '$srchtxtsql%' ORDER BY albumid DESC LIMIT ".$_G['setting']['search']['album']['maxsearchresults']);
			while($album = $sdb->fetch_array($query)) {
				$ids .= ','.$album['albumid'];
				$num++;
			}
