 ...
 *
 *      $Id: switch.inc.php 9047 2010-04-26 08:45:00Z monkey $
 */
...
if(!in_array($_G['groupid'], $myrepeatsusergroups)) {
	...
	if(!DB::num_rows($query)) {
		...
		if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_member')." WHERE uid IN (".dimplode($permusers).") AND username='$_G[gp_username]'")) {
			showmessage('myrepeats:usergroup_disabled');
		}
	}
}
...
if($result['status'] > 0) {
	...
	if(!DB::result_first("SELECT COUNT(*) FROM ".DB::table('myrepeats')." WHERE uid='$_G[uid]' AND username='$olddiscuz_user'")) {
		...
		showmessage('myrepeats:login_succeed_rsnonexistence', '', array('user' => $discuz_userss, 'comment' => $comment, 'olduser' => $olddiscuz_userss, 'olduserenc' => $olddiscuz_userssenc, 'ucsynlogin' => $ucsynlogin, 'referer' => $referer));
	} else {
		showmessage('myrepeats:login_succeed', $referer, array('user' => $_G['member']['username'], 'comment' => $comment, 'ucsynlogin' => $ucsynlogin));
	}
} elseif($result['status'] == -1) {
	$ucresult['username'] = addslashes($ucresult['username']);
	$auth = authcode("$ucresult[username]\t".FORMHASH, 'ENCODE');
	showmessage('myrepeats:login_activation', 'member.php?mod=register&action=activation&auth='.rawurlencode($auth).'&referer='.rawurlencode($referer), array('user' => $username));
} else {
