...
if($operation == 'illegal') {
	...
	showtablerow('class="header"', array('class="td23"','class="td23"','class="td23"','class="td24"','class="td24"'), array(
		...
		cplang('time'),
		cplang('logs_credit_send'),
		cplang('logs_credit_receive'),
		cplang('action'),
	));
	...
	$keywordadd = !empty($_G['gp_keyword']) ? "AND c.fromto LIKE '%{$_G['gp_keyword']}%'" : '';

	$mpurl = ADMINSCRIPT."?action=logs&operation=$operation&keyword=".rawurlencode($_G['gp_keyword'])."&lpp=$lpp";
	if(in_array($_G['gp_opt'], array('TFR', 'RCV', 'EXC', 'UGP', 'AFD'))) {
		$optadd = "AND c.operation='{$_G['gp_opt']}'";
		$mpurl .= '&opt='.$_G['gp_opt'];
	} else {
		$optadd = '';
	}

	$num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_credit_rule_log')." c WHERE 1 $keywordadd $optadd");

	$multipage = multi($num, $lpp, $page, $mpurl, 0, 3);

	$total['send'] = $total['receive'] = array();
	$query = DB::query("SELECT c.*, m.username FROM ".DB::table('common_credit_rule_log')." c
	...
	while($log = DB::fetch($query)) {
		...
		showtablerow('', array('class="bold"'), array(
			...
			$log[fromto],
			$log[dateline],
			isset($_G['setting']['extcredits'][$log['sendcredits']]) ? $_G['setting']['extcredits'][$log['sendcredits']]['title'].' '.$log['send'].' '.$_G['setting']['extcredits'][$log['sendcredits']]['unit'] : $log['send'],
			isset($_G['setting']['extcredits'][$log['receivecredits']]) ? $_G['setting']['extcredits'][$log['receivecredits']]['title'].' '.$log['receive'].' '.$_G['setting']['extcredits'][$log['receivecredits']]['unit'] : $log['receive'],
			$log[operation],
		));
	}
	...
	if(!submitcheck('invitesubmit')) {
		...
		while($invite = DB::fetch($query)) {
			...
			$invite['expiration'] = dgmdate($invite['expiration'], 'Y-n-j H:i');
			$stats = $invite['statuslog'].($invite['status'] == 2 ? '&nbsp;[<a href="home.php?mod=space&uid='.$invite['reguid'].'">'.$lang['logs_invite_target'].'</a>]' : '');
		...
		if($deletelist = dimplode($delete)) {
			DB::query("DELETE FROM ".DB::table('common_invite')." WHERE invitecode IN ($deletelist)");
		}
	...
	$query = DB::query("SELECT l.*, m.username, t.subject, t.dateline AS postdateline, t.author, t.authorid AS tauthorid
			...
			WHERE operation='BTC'
			ORDER BY l.dateline LIMIT $start_limit,$tpp");
