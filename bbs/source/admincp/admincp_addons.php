 ...
 *
 *      $Id: admincp_addons.php 9651 2010-04-30 09:54:22Z monkey $
 */
...
if(!$operation) {
	...
	$baseparm = 'version='.rawurlencode(DISCUZ_VERSION).'&release='.rawurlencode(DISCUZ_RELEASE).'&charset='.rawurlencode(CHARSET).'&boardurl='.rawurlencode($_G['siteurl']);
	...
	if($data['Searchlink'] != '') {
		echo '<form method="post" autocomplete="off" action="'.$data['Searchlink'].'" target="_blank">'.
			...
			'<input type="hidden" name="boardurl" value="'.$_G['siteurl'].'" />'.
	...
	if(empty($_G['gp_providerkey'])) {
		...
		if(is_array($data) && $data) {
			...
			echo '<tr><td>';
			foreach($data as $row) {
				echo '<div class="hover" style="float:left;width:20%"><div style="text-align:center"><a href="'.ADMINSCRIPT.'?action=addons&operation=add&providerkey='.$row['key'].'">'.
		...
		if($addon) {
		cpmsg('addons_provider_exists', 'action=addons&operation=list&provider='.rawurlencode($_G['gp_providerkey']), 'succeed');
		}
		require_once DISCUZ_ROOT.'./source/discuz_version.php';
		...
		cpmsg('addons_provider_addsucceed', 'action=addons&operation=list&provider='.rawurlencode($_G['gp_providerkey']), 'succeed');
	}
...
function showproviderinfo($addon, $simple) {
	...
	echo (!$simple ? '<tr><th colspan="3" class="partition"><a href="'.ADMINSCRIPT.'?action=addons&operation=list&provider='.$addon['key'].'">'.$addon['title'].'</a></th></tr>' : '').
		'<tr><td width="110"><a href="'.ADMINSCRIPT.'?action=addons&operation=list&provider='.$addon['key'].'"><img onerror="this.src=\'static/image/common/none.gif\'" src="'.$addon['logo'].'" width="100" height="50" /></a></td>'.
		'<td valign="top">'.nl2br($addon['description']).'<br />'.
		cplang('addons_provider').'<a href="'.$addon['siteurl'].'" target="_blank">'.$addon['sitename'].'</a><br />'.
