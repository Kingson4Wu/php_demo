 ...
 *
 *      $Id: admincp_recyclebin.php 6757 2010-03-25 09:01:29Z cnteacher $
 */
...
if(!$operation) {
	...
	if(!submitcheck('delsubmit') && !submitcheck('undelsubmit')) {
		...
		$lpp = empty($_G['gp_lpp']) ? 10 : $_G['gp_lpp'];
		$start = ($page - 1) * $lpp;
...
EOT;
		...
		if(submitcheck('searchsubmit')) {
			...
			while($thread = DB::fetch($query)) {
				...
				showtablerow("id=\"mod_$thread[tid]_row3\"", 'class="threadopt threadtitle" colspan="2"', "$lang[operator]: <a href=\"home.php?mod=space&uid=$thread[moduid]\" target=\"_blank\">$thread[modusername]</a> &nbsp;&nbsp; $lang[recyclebin_delete_time]: $thread[moddateline]");
			}
...
<?
	...
	if(!submitcheck('rbsubmit')) {
		...
		$query = DB::query("SELECT tm.tid FROM ".DB::table('forum_threadmod')." tm, ".DB::table('forum_thread')." t
			WHERE tm.dateline<'$timestamp-$_G[gp_days]*86400' AND tm.action='DEL' AND t.tid=tm.tid AND t.displayorder='-1'");
		while($thread = DB::fetch($query)) {
