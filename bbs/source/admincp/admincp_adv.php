 ...
 *
 *      $Id: admincp_adv.php 10591 2010-05-12 11:01:58Z monkey $
 */
...
if($operation == 'ad') {
	...
	if(!submitcheck('advsubmit')) {
		...
		$multipage = multi($advnum, $advppp, $page, ADMINSCRIPT.'?action=adv&operation=ad'.($title ? '&title='.rawurlencode($title) : '').($starttime ? "&starttime=$starttime" : '').($endtime ? "&endtime=$endtime" : '').($orderby ? "&orderby=$orderby" : ''), 0, 3, TRUE, TRUE);
