 ...
 *
 *      $Id: admincp_pic.php 10915 2010-05-18 04:48:02Z monkey $
 */
...
if(submitcheck('searchsubmit')) {
	...
	if(!$error) {
		if($detail) {
			$pagetmp = $page;
			do{
				$query = DB::query("SELECT * FROM ".DB::table('home_pic')." p LEFT JOIN ".DB::table('home_album')." a USING(albumid) WHERE 1 $sql ORDER BY p.dateline DESC LIMIT ".(($pagetmp - 1) * $ppp).",{$ppp}");
				$pagetmp--;
