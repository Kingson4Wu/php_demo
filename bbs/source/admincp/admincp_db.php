 ...
 *
 *      $Id: admincp_db.php 10922 2010-05-18 06:28:40Z monkey $
 */
...
if(!$backupdir = DB::result_first("SELECT svalue FROM ".DB::table('common_setting')." WHERE skey='backupdir'")) {
	$backupdir = random(6);
...
if($operation == 'export') {
	...
	if(!submitcheck('importsubmit', 1) && !submitcheck('deletesubmit')) {
		...
		if($dumpinfo['method'] == 'multivol') {
			...
			if($_G['gp_from'] == 'local') {
				cpmsg('database_import_file_succeed', ADMINSCRIPT.'?action=db&operation=import', 'succeed');
			}
	...
	if($step == 3) {
		...
		if(submitcheck('repairsubmit') && (!empty($repair) || !empty($setting) || !empty($repairtable) || !empty($missingtable))) {
			...
			if(!empty($setting)) {
				...
				$settingsquery = DB::query("SELECT skey FROM ".DB::table('common_setting')." WHERE SUBSTRING(skey, 1, 9)<>'jswizard_' ORDER BY skey");
