 ...
 *
 *      $Id: admincp_threads.php 10592 2010-05-12 11:03:52Z zhaoxiongfei $
 */
...
EOT;
	...
	if(submitcheck('searchsubmit')) {
		...
		$sql = '';
		$sql .= $_G['gp_inforum'] != '' && $_G['gp_inforum'] != 'all' ? " AND fid='{$_G['gp_inforum']}'" : '';
		...
		if($sql) {
			...
			if($_G['gp_detail']) {
				$pagetmp = $page;
				do {
					$query = DB::query("SELECT fid, tid, readperm, price, subject, authorid, author, views, replies, lastpost FROM ".DB::table('forum_thread')." WHERE $sql LIMIT ".(($pagetmp - 1) * $_G['setting']['topicperpage']).",{$_G['setting']['topicperpage']}");
					$pagetmp--;
				...
				while($thread = DB::fetch($query)) {
					...
					$threads .= showtablerow('', array('class="td25"', '', '', '', 'class="td25"', 'class="td25"'), array(
						...
						"<a href=\"forum.php?mod=viewthread&tid=$thread[tid]\" target=\"_blank\">$thread[subject]</a>".($thread['readperm'] ? " - [$lang[threads_readperm] $thread[readperm]]" : '').($thread['price'] ? " - [$lang[threads_price] $thread[price]]" : ''),
				...
				$multi = multi($threadcount, $_G['setting']['topicperpage'], $page, ADMINSCRIPT."?action=threads");
		...
		showsubmit('modsubmit', 'submit', '<input name="chkall" id="chkall" type="checkbox" class="checkbox" checked="checked" onclick="checkAll(\'prefix\', this.form, \'tidarray\', \'chkall\')" /><label for="chkall">'.cplang('select_all').'</label>', '', $multi);
		showtablefooter();
		showformfooter();
	...
	$tidsadd = isset($_G['gp_tids']) ? 'tid IN ('.$tids.')' : 'tid IN ('.dimplode($_G['gp_tidarray']).')';

	if($operation == 'moveforum') {

		if(!DB::result_first("SELECT fid FROM ".DB::table('forum_forum')." WHERE type<>'group' AND fid='{$_G['gp_toforum']}'")) {
			cpmsg('threads_move_invalid', '', 'error');
		}

		DB::query("UPDATE ".DB::table('forum_thread')." SET fid='{$_G['gp_toforum']}' WHERE $tidsadd");
		...
		if($_G['gp_totype'] != 0) {
			if(!DB::result_first("SELECT typeid FROM ".DB::table('forum_threadtype')." WHERE typeid='{$_G['gp_totype']}' AND special='0'")) {
				cpmsg('threads_move_invalid', '', 'error');
			}
		}
		...
		if(!submitcheck('forumsticksubmit')) {
			...
			if($do == 'del') {
				...
				if(preg_match('/tid=(\d+)|thread-(\d+)-/i', $_G['gp_forumstick_url'], $matches)) {
		...
		if(!$do) {
			...
			if(submitcheck('addpositionsubmit', 1)) {
				$delete = isset($_G['gp_delete']) && is_array($_G['gp_delete']) ? $_G['gp_delete'] : explode(',', $_G['gp_tids']);
				if(empty($delete)) {
	...
	$cpmsg = $_G['gp_cpmsg'] ? "alert('$_G[gp_cpmsg]]');" : '';
