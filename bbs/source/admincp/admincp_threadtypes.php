 ...
 *
 *      $Id: admincp_threadtypes.php 10231 2010-05-07 10:16:43Z monkey $
 */
...
if(!$operation) {
	...
	if(!submitcheck('typesubmit')) {
		...
		$threadtypes = '';
		$query = DB::query("SELECT * FROM ".DB::table('forum_threadtype')." WHERE ".($special ? "special!='0'" : "special='0'")." ORDER BY displayorder");
		while($type = DB::fetch($query)) {
			$threadtypes .= showtablerow('', array('class="td25"', 'class="td28"', '', 'class="td29"', 'title="'.cplang('forums_threadtypes_forums_comment').'"', 'class="td25"'), array(
				...
				$special ? "<a href=\"".ADMINSCRIPT."?action=threadtypes&operation=sortdetail&sortid=$type[typeid]\" class=\"act nowrap\">$lang[detail]</a>" : ''
			), TRUE);
		}
...
<?
		...
		if(is_array($_G['gp_delete'])) {
			...
			if($deleteids = dimplode($_G['gp_delete'])) {
				...
				DB::query("DELETE FROM ".DB::table('forum_threadtype')." WHERE typeid IN ($deleteids) AND special='$special'");
			}
		...
		if(is_array($_G['gp_namenew']) && $_G['gp_namenew']) {
			foreach($_G['gp_namenew'] as $typeid => $val) {
				DB::update('forum_threadtype', array(
					...
					'displayorder' => $_G['gp_displayorder'][$typeid],
					'special' => $special,
				), "typeid='$typeid'");
				if(DB::affected_rows()) {
		...
		cpmsg('forums_threadtypes_succeed', 'action=threadtypes&special='.$special, 'succeed');
