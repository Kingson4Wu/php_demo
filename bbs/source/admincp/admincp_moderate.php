 ...
 *
 *      $Id: admincp_moderate.php 10846 2010-05-17 06:59:35Z zhaoxiongfei $
 */
...
if($operation == 'threads') {
	...
	if(!submitcheck('modsubmit')) {
		...
		if($validatepids = dimplode($moderation['validate'])) {
			...
			$postarray = getallwithposts(array(
				'select' => 't.lastpost, p.pid, p.fid, p.tid, p.authorid, p.author, p.dateline, p.attachment, p.message, p.anonymous, ff.replycredits',
				'from' => DB::table('forum_post')." p LEFT JOIN ".DB::table('forum_forumfield')." ff ON ff.fid=p.fid LEFT JOIN ".DB::table('forum_thread')." t ON t.tid=p.tid",
