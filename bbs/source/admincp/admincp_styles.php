 ...
 *
 *      $Id: admincp_styles.php 10770 2010-05-14 10:44:11Z monkey $
 */
...
if($operation == 'admin') {
	...
	if(!submitcheck('stylesubmit')) {
		...
		foreach($sarray as $id => $style) {
			...
			$stylelist .= ($i == 0 ? '<tr>' : '').
				...
				($id > 0 ? "<p style=\"margin-bottom: 2px;\">&nbsp;</p><img src=\"$preview\" alt=\"$lang[preview]\"/></a>
				<p style=\"margin: 2px 0\"><span style='float: left; dispaly: inline; margin-left: 4px; width: 20px; height: 20px; ".($_G['setting']['styleicons'][$id] ? $_G['setting']['styleicons'][$id] : 'background: url(./static/image/admincp/transparent.gif)')."'>&nbsp;</span><input type=\"text\" class=\"txt\" name=\"namenew[$id]\" value=\"$style[name]\" size=\"30\" style=\"margin-right:0; width: 80px;\"></p>
	...
	if(!submitcheck('importsubmit') && !isset($_G['gp_dir'])) {
		...
		if($restore) {
			$_G['gp_dir'] = DB::result_first("SELECT t.directory FROM ".DB::table('common_style')." s LEFT JOIN ".DB::table('common_template')." t ON t.templateid=s.templateid WHERE styleid='$restore'");
		}
		if(!empty($_G['gp_dir'])) {
	...
	if(!submitcheck('editsubmit')) {
		...
		$style = DB::fetch_first("SELECT name, templateid FROM ".DB::table('common_style')." WHERE styleid='$id'");
		if(!$style) {
...
<?
		...
		if($_G['gp_newcvar'] && $_G['gp_newcsubst']) {
			if(DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_stylevar')." WHERE variable='$_G[gp_newcvar]' AND styleid='$id'")) {
		...
		DB::query("UPDATE ".DB::table('common_style')." SET name='$namenew', templateid='$templateidnew' WHERE styleid='$id'");
