 ...
 *
 *      $Id: admincp_medals.php 9035 2010-04-26 07:37:43Z monkey $
 */
...
<?
		...
		echo 'result = result.replace(/and/g, \'&nbsp;&nbsp;'.$lang['setting_formulaperm_and'].'&nbsp;&nbsp;\');';
		echo 'result = result.replace(/or/g, \'&nbsp;&nbsp;'.$lang['setting_formulaperm_or'].'&nbsp;&nbsp;\');';
...
<?
		...
		$_G['gp_formulapermnew'] = str_replace(' ', '', $_G['gp_formulapermnew']);
		if(!checkformulaperm($_G['gp_formulapermnew'])) {
			cpmsg('forums_formulaperm_error', '', 'error');
		}

		$formulapermary[0] = $_G['gp_formulapermnew'];
		$formulapermary[1] = preg_replace("/(digestposts|posts|threads|extcredits[1-8])/", "getuserprofile('\\1')", $_G['gp_formulapermnew']);
