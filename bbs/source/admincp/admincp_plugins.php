 ...
 *
 *      $Id: admincp_plugins.php 10922 2010-05-18 06:28:40Z monkey $
 */
...
if(!$operation) {
	...
	if(!submitcheck('submit')) {
		...
		if(is_array($availablenew)) {
			$query = DB::query("SELECT pluginid, modules FROM ".DB::table('common_plugin')." WHERE pluginid IN (".dimplode(array_keys($availablenew)).")");
			while($plugin = DB::fetch($query)) {
	...
	if(!empty($plugin['directory']) && !empty($pluginarray['upgradefile']) && preg_match('/^[\w\.]+$/', $pluginarray['upgradefile'])) {
		dheader('location: '.ADMINSCRIPT.'?action=plugins&operation=pluginupgrade&dir='.$plugin['directory'].'&xmlfile='.rawurlencode($xmlfile).'&fromversion='.$plugin['version']);
	}
	$toversion = $pluginarray['plugin']['version'];
			...
			if($pluginvars) {
				...
				foreach($pluginvars as $var) {
					...
					if($var['type'] == 'number') {
						...
						$var['type'] = 'calendar';
						$extra['date'] = '<script type="text/javascript" src="static/js/forum_calendar.js"></script>';
	...
	if(!submitcheck('editsubmit')) {
		...
		if(is_array($plugin['modules'])) {
			foreach($plugin['modules'] as $moduleid => $module) {
				...
				$typeselect = '<optgroup label="'.cplang('plugins_edit_modules_type_g1').'">'.
					'<option h="1100" e="inc" value="1"'.($module['type'] == 1 || $module['type'] == 2 ? ' selected="selected"' : '').'>'.cplang('plugins_edit_modules_type_1').'</option>'.
					'<option h="1111" e="inc" value="5"'.($module['type'] == 5 || $module['type'] == 6 ? ' selected="selected"' : '').'>'.cplang('plugins_edit_modules_type_5').'</option>'.
					'</optgroup>'.
		...
		showtablerow('', array('class="td25"', 'class="td28"'), array(
			...
			'<select id="s_n" onchange="shide(this, \'n\')" name="newtype">'.
				...
				'<option h="1100" e="inc" value="1">'.cplang('plugins_edit_modules_type_1').'</option>'.
			...
			if(is_array($plugin['modules'])) {
				foreach($plugin['modules'] as $moduleid => $module) {
					if(!isset($_G['gp_delete'][$moduleid])) {
						...
						if($_G['gp_typenew'][$moduleid] == 1) {
							$updatenav = true;
						}
					} elseif($_G['gp_typenew'][$moduleid] == 1) {
						$updatenav = true;
					}
				}
			}
	...
	if(!submitcheck('varsubmit')) {
		...
		$typeselect = '<select name="typenew" onchange="if(this.value == \'select\' || this.value == \'selects\') $(\'extra\').style.display=\'\'; else $(\'extra\').style.display=\'none\';">';
		foreach(array('number', 'text', 'radio', 'textarea', 'select', 'selects', 'color', 'date', 'datetime', 'forum', 'forums', 'group', 'groups', 'extcredit') as $type) {
...
function updatemenu() {
	...
	foreach($pluginmenus as $menu) {
		if($menu[0] && $menu[1]) {
			$s .= '<li><a href="'.(substr($menu[1], 0, 4) == 'http' ? $menu[1] : ADMINSCRIPT.'?action='.$menu[1]).'" hidefocus="true" target="'.($menu[2] ? $menu[2] : 'main').'"'.($menu[3] ? $menu[3] : '').'>'.cplang($menu[0]).'</a></li>';
		}
	}
...
function pluginstat($type, $data) {
	global $_G;
	$url = 'http://stat.discuz.com/plugins.php?action='.$type.'&id='.rawurlencode('[X1]'.$data['identifier']).'&version='.rawurlencode($data['version']).'&url='.rawurlencode($_G['siteurl']).'&ip='.$_G['clientip'];
