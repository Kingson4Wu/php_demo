 ...
 *
 *      $Id: admincp_magics.php 9035 2010-04-26 07:37:43Z monkey $
 */
...
if($operation == 'admin') {
	...
	if(!submitcheck('magicsubmit')) {
		...
		$newmagics = getmagics();
		showsubtitle(array('', 'display_order', 'available', 'name', $lang['price'].'('.$_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['title'].')', 'magics_num', 'weight', 'magics_identifier'));
		$unit = $_G['setting']['extcredits'][$_G['setting']['creditstransextra'][3]]['unit'];

		$query = DB::query("SELECT * FROM ".DB::table('common_magic')." $addtype ORDER BY displayorder");
		while($magic = DB::fetch($query)) {
			...
			showtablerow('', array('class="td25"', 'class="td25"', 'class="td25"', 'class="td28"', 'class="td28"', 'class="td28"', 'class="td28"', '', ''), array(
				...
				"<input type=\"text\" class=\"txt\" name=\"price[$magic[magicid]]\" value=\"$magic[price]\">".$unit,
				...
				"<input type=\"text\" class=\"txt\" name=\"weight[$magic[magicid]]\" value=\"$magic[weight]\">",
		...
		foreach($newmagics as $newmagic) {
			showtablerow('', array('class="td25"', 'class="td25"', 'class="td25"', 'class="td28"', 'class="td28"', 'class="td28"', 'class="td28"', '', ''), array(
				...
				"<input type=\"text\" class=\"txt\" name=\"newprice[$newmagic[class]]\" value=\"$newmagic[price]\">".$unit,
		...
		if(is_array($_G['gp_name'])) {
			foreach($_G['gp_name'] as $id => $val) {
				DB::query("UPDATE ".DB::table('common_magic')." SET available='".$_G['gp_available'][$id]."', name='$val', identifier='".$_G['gp_identifier'][$id]."', displayorder='".$_G['gp_displayorder'][$id]."', price='".$_G['gp_price'][$id]."', num='".$_G['gp_num'][$id]."', weight='".$_G['gp_weight'][$id]."' WHERE magicid='$id'");
			}
		}
