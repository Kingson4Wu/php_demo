 ...
 *
 *      $Id: admincp_smilies.php 10160 2010-05-07 05:42:53Z monkey $
 */
...
if($operation == 'export' && $id) {
	...
	$query = DB::query("SELECT typeid, displayorder, code, url FROM ".DB::table('common_smiley')." WHERE type='smiley' AND typeid='$id'");
...
if(!$operation) {
	...
	if(!$do) {
		...
		if(!submitcheck('editsubmit')) {
			...
			$num = DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_smiley')." WHERE type='smiley' AND typeid='$id'");
			$multipage = multi($num, $smiliesperpage, $page, 'action=smilies&operation=edit&id='.$id);

			$smileynum = 1;
			$smilies = '';
			$query = DB::query("SELECT * FROM ".DB::table('common_smiley')." WHERE type='smiley' AND typeid='$id' ORDER BY displayorder LIMIT $start_limit, $smiliesperpage");
			while($smiley =	DB::fetch($query)) {
...
EOT;
		...
		if(!submitcheck('editsubmit')) {
			...
			if($_G['gp_search']) {
				...
				$newimages = '';
				$imgfilter =  array();
				$query = DB::query("SELECT url FROM ".DB::table('common_smiley')." WHERE type='smiley' AND typeid='$id'");
