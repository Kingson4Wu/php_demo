 ...
 *
 *      $Id: admincp_block.php 10915 2010-05-18 04:48:02Z monkey $
 */
...
if($operation=='perm') {
	...
	if($operation=='list') {
		showsubmenu('block',  array(
				array('list', 'block', 1),
				array('block_jscall', 'block&operation=jscall', 0)
			));
	} else {
		$addmenu = array();
		foreach($_G['cache']['blockclass'] as $key=>$value) {
			foreach($value['subs'] as $subkey=>$subvalue) {
				$addmenu[] = array($subvalue['name'], 'blockstyle&operation=add&blockclass='.$subkey, 'showWindow(\'showblock\',\'portal.php?mod=portalcp&ac=block&op=add&classname='.$subkey.'&blocktype=1\');return false;');
			}
		}
		showsubmenu('block',  array(
				array('list', 'block', 0),
				array('block_jscall', 'block&operation=jscall', 1),
				array(array('menu'=>'block_add_jscall', 'submenu'=>$addmenu), '', $operation=='add' || $operation=='edit')
			));
	}
	...
	if(submitcheck('deletesubmit')) {
		...
		'block_dateline', 'block_id', 'block_name', 'block_blockclass');
		foreach ($keys as $key) {
			...
			<form method="get" autocomplete="off" action="$adminscript">
				<div style="margin-top:8px;">
					<table cellspacing="3" cellpadding="3">
						...
						<tr>
							<th>$searchlang[block_blockclass]</th><td>$blockclass_sel</td>
							<th>&nbsp;</td>
						</tr>
						<tr>
