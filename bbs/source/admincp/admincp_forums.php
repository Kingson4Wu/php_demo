 ...
 *
 *      $Id: admincp_forums.php 10902 2010-05-18 02:57:04Z liulanbo $
 */
...
var rowtypedata = [
	[[1,'<input type="text" class="txt" name="newcatorder[]" value="0" />', 'td25'], [3, '<input name="newcat[]" value="<?php cplang('forums_admin_add_category_name', null, true);?>" size="20" type="text" class="txt" />']],
	[[1,'<input type="text" class="txt" name="neworder[{1}][]" value="0" />', 'td25'], [3, '<div class="board"><input name="newforum[{1}][]" value="<?php cplang('forums_admin_add_forum_name', null, true);?>" size="20" type="text" class="txt" /></div>']],
	[[1,'<input type="text" class="txt" name="neworder[{1}][]" value="0" />', 'td25'], [3, '<div class="childboard"><input name="newforum[{1}][]" value="<?php cplang('forums_admin_add_forum_name', null, true);?>" size="20" type="text" class="txt" />&nbsp;<label><input name="inherited[{1}][]" type="checkbox" class="checkbox" value="1">&nbsp;<?php cplang('forums_edit_inherited', null, true);?></label></div>']],
];
</script>
<?
		showformheader('forums');
		showtableheader('');
		showsubtitle(array('display_order', 'forums_admin_name', 'forums_moderators', '<a href="javascript:;" onclick="if(getmultiids()) location.href=\''.ADMINSCRIPT.'?action=forums&operation=edit&multi=\' + getmultiids()">'.$lang['multiedit'].'</a>'));
		...
		$table_forumfield_columns = array('fid', 'attachextensions', 'threadtypes', 'viewperm', 'postperm', 'replyperm', 'getattachperm', 'postattachperm', 'postimageperm');
		...
		if(is_array($_G['gp_newforum'])) {
			foreach($_G['gp_newforum'] as $fup => $forums) {
				...
				foreach($forums as $key => $forumname) {
					...
					$forumfields = array();
					$inheritedid = $_G['gp_inherited'][$fup] ? $fup : '';


					if(!empty($inheritedid)) {

						$query = DB::query("SELECT * FROM ".DB::table('forum_forum')." WHERE fid='$inheritedid'");
						$forum = DB::fetch($query);

						$query = DB::query("SELECT * FROM ".DB::table('forum_forumfield')." WHERE fid='$inheritedid'");
						$forumfield = DB::fetch($query);
					...
					while($mod = DB::fetch($query)) {
						if($mod['inherited'] || $forum['inheritedmod']) {
							...
							DB::query("REPLACE INTO ".DB::table('forum_moderator')." (uid, fid, inherited)
	...
	if(!submitcheck('detailsubmit')) {
		$anchor = in_array($_G['gp_anchor'], array('basic', 'extend', 'posts', 'credits', 'threadtypes', 'threadsorts', 'perm')) ? $_G['gp_anchor'] : 'basic';
		...
		$forumselect = '<span id="fselect" class="right popupmenu_dropmenu" onmouseover="showMenu({\'ctrlid\':this.id,\'pos\':\'34\'});$(\'fselect_menu\').style.top=(parseInt($(\'fselect_menu\').style.top)-document.documentElement.scrollTop)+\'px\';$(\'fselect_menu\').style.left=(parseInt($(\'fselect_menu\').style.left)-document.documentElement.scrollLeft-20)+\'px\'">'.cplang('forums_edit_switch').'<em>&nbsp;&nbsp;</em></span>'.
			'<div id="fselect_menu" class="popupmenu_popup" style="display:none"><div><span class="pointer right" onclick="$(\'menuform\').submit()"><b>'.cplang('forums_multiedit').'</b></span>'.$forumselect.'</div></div>';
		...
		if(count($mforum) == 1 && $mforum[0]['type'] == 'group') {
			...
			showsubmenuanchors(cplang('forums_edit').(count($mforum) == 1 ? ' - '.$mforum[0]['name'] : ''), array(
				...
				!$multiset ? array('forums_edit_threadtypes', 'threadtypes', $anchor == 'threadtypes') : array(),
				!$multiset ? array('forums_edit_threadsorts', 'threadsorts', $anchor == 'threadsorts') : array(),
		...
		if(count($mforum) == 1 && $mforum[0]['type'] == 'group') {
			...
			if($multiset) {
				...
				foreach($mforum as $forum) {
					$_G['showtableheader_multi'][] = '<a href="javascript:;" onclick="location.href=\''.ADMINSCRIPT.'?action=forums&operation=edit&fid='.$forum['fid'].'&anchor=\'+$(\'cpform\').anchor.value;return false">'.$forum['name'].'</a>';
				}
			}
			$mfids = array();
			foreach($mforum as $forum) {
			...
			showsetting('forums_edit_basic_display', array('statusnew', array(
					array(1, cplang('forums_edit_basic_display_yes')),
					array(0, cplang('forums_edit_basic_display_no')),
					array(2, cplang('forums_edit_basic_display_select'))
			)), $forum['status'], 'mradio');
			if(!$multiset) {
...
EOT;
					...
					echo $typeselect;
					echo '<tr><td colspan="6"><div><a href="###" onclick="addrow(this, 0)" class="addtr">'.cplang('threadtype_infotypes_add').'</a></div></td></tr>';
					showtablefooter();
					showtagfooter('div');
	...
	showformfooter();
		...
		if(!$multiset) {
			...
			$formulapermary[1] = preg_replace(
				array("/(digestposts|posts|threads|extcredits[1-8])/", "/(regdate|regday|regip|lastip|buyercredit|sellercredit|field\d+)/"),
		...
		if($_G['gp_type'] == 'group') {
			...
			$allowspecialonlynew = $_G['gp_allowpostspecialnew'] || $_G['setting']['threadplugins'] && $threadpluginnew ? $_G['gp_allowspecialonlynew'] : 0;
			$forumcolumnsnew = $_G['gp_forumcolumnsnew'] > 1 ? intval($_G['gp_forumcolumnsnew']) : 0;
			...
			if(!$multiset) {
				...
				foreach($_G['gp_creditnew'] as $rid => $rule) {
					$creditspolicynew[$rules[$rid]['action']] = $rules[$rid];
					$check = 0;
					foreach($rule as $i => $v) {
