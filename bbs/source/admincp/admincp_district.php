...
if(submitcheck('editsubmit')) {
	...
	foreach($thevalues as $value) {
		showtablerow('id="td_'.$value[0].'"', array('', '', ''), array(
			'',
			'<p id="p_'.$value[0].'">'
			.'<input type="text" id="input_'.$value[0].'" name="district['.$value[0].']" value="'.$value[1].'" style="display: none;" />'
	...
	showtablerow('id="td_new"', array('', '', ''), array(
			'',
			'<input type="text" id="input_new" value="" name="districtnew[]"/>&nbsp;&nbsp;<a href="javascript:;" onclick="adddistrict();return false;">'.cplang('add').'</a>'
			.'<div id="div_new"></div>',
			''
		));
...
function adddistrict() {
	...
	elem.innerHTML = '<input type="text" name="districtnew[]" value="' + i.value + '" />';
	$('div_new').appendChild(elem);
	i.value = '';
}
</script>
SCRIPT;
