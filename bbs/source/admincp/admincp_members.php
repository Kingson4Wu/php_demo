 ...
 *
 *      $Id: admincp_members.php 10931 2010-05-18 07:38:22Z zhengqingpeng $
 */
...
if($operation == 'search') {
	...
	if(!submitcheck('submit', 1)) {
		...
		if(!$_G['gp_confirmed']) {
			...
			cpmsg('members_delete_confirm', "action=members&operation=clean&submit=yes&confirmed=yes".$urladd, 'form', array('membernum' => $membernum), $extra.'<br /><input type="checkbox" name="includepost" value="1" class="checkbox" />'.$lang['members_delete_post'].($isfounder ? '&nbsp;<input type="checkbox" name="includeuc" value="1" class="checkbox" />'.$lang['members_delete_ucdata'] : ''), '');
			...
			if(empty($_G['gp_includepost'])) {
				...
				$nextlink = ADMINSCRIPT."?action=members&operation=clean&confirmed=yes&submit=yes&includepost=yes&current=$next&pertask=$pertask&lastprocess=$processed".$urladd;
				if($threads) {
					...
					cpmsg(cplang('members_delete_post').': '.cplang('members_delete_processing', array('current' => $current, 'next' => $next)), $nextlink, 'loadingform', array(), $extra);
...
EOT;
	...
	if(!empty($_G['gp_username']) || !empty($_G['gp_uid'])) {
		$member = DB::fetch_first("SELECT m.*, mf.*, u.type AS grouptype, uf.allowsigbbcode, uf.allowsigimgcode FROM ".DB::table('common_member')." m
			LEFT JOIN ".DB::table('common_member_field_forum')." mf ON mf.uid=m.uid
			LEFT JOIN ".DB::table('common_usergroup')." u ON u.groupid=m.groupid
	...
	if(!submitcheck('bansubmit')) {
		...
		if($member) {
			showtablerow('', 'class="td27"', cplang('members_edit_current_status').'<span class="normal">: '.($member['groupid'] == 4 ? $lang['members_ban_post'] : ($member['groupid'] == 5 ? $lang['members_ban_visit'] : $lang['members_ban_none'])).'</span>');
		}
		...
		if($_G['group']['allowbanuser'] && (DB::affected_rows())) {
			banlog($member['username'], $member['groupid'], $groupidnew, $_G['gp_banexpirynew'], $reason);
		}
		...
		if($_G['gp_delpost'] && $_G['gp_bannew'] && $_G['adminid'] == 1) {
	...
	$member = DB::fetch_first("SELECT m.*, mf.*, mc.*, mh.*, ms.*, mp.*, m.uid AS muid, u.type, uf.allowsigbbcode, uf.allowsigimgcode, uf.allowcusbbcode
		FROM ".DB::table('common_member')." m
		LEFT JOIN ".DB::table('common_member_field_forum')." mf ON mf.uid=m.uid
	...
	if(!submitcheck('editsubmit')) {
		...
		showsetting('members_edit_status', 'status', $member['status'], '<input class="radio" type="radio" name="status" value="-1" '.$status[-1].'> '.$lang['yes'].' <input class="radio" type="radio" name="status" value="0" '.$status[0].'> '.$lang['no']);
	...
	if($fieldid) {
		...
		if(!submitcheck('editsubmit')) {
			...
			if(!$field['isfixed2']) {
				...
				showtagheader('tbody', 'fieldchoices', !in_array($field['formtype'], array('text', 'textarea')), 'sub');
			...
			showsetting('members_profile_edit_default_privacy', array('privacy', $privacyselect), $_G['setting']['privacy'][$fieldid], 'select');
			...
			if($_G['setting']['privacy'][$fieldid] != $_POST['privacy']) {
				$_G['setting']['privacy'][$fieldid] = $_POST['privacy'];
				DB::insert('common_setting', array('skey'=>'privacy', 'svalue'=>serialize($_G['setting']['privacy'])), array('skey'=>'privacy'), false, true);
				updatecache('setting');
			}
	...
	if(submitcheck('securitysubmit')) {
		if(empty($_POST['do']) || !in_array($_POST['do'], array('pass', 'ignore'))) {
			cpmsg('members_verify_null_operation', 'action=members&operation=verify&page='.$page, 'error');
		}
		if(empty($_POST['ids'])) {
			cpmsg('members_verify_null_selection', 'action=members&operation=verify&page='.$page, 'error');
		} else {
			if($_POST['do'] == 'pass') {
				$plus_group = $plus_credit = array();
				if(in_array('group', $_POST['plus'])) {
					$_POST['groupid'] = intval($_POST['groupid']);
					$group = DB::fetch_first('SELECT * FROM '.DB::table('common_usergroup')." WHERE groupid='$_POST[groupid]'");
					if($group) {
						$plus_group = $group;
					}
				}
				if(in_array('credit', $_POST['plus'])) {
					if(in_array($_POST['extcredits'], range(1,8)) && $_POST['credits'] > 0) {
						$plus_credit = array('extcredits'=>'extcredits'.$_POST['extcredits'], 'credits'=>intval($_POST['credits']));
					}
				}
				$updated_members = array();
				$query = DB::query('SELECT * FROM '.DB::table('common_member_security')." WHERE securityid IN (".dimplode($_POST['ids']).")");
				while($value = DB::fetch($query)) {
					$value = daddslashes($value);
					DB::update('common_member_profile', array($value['fieldid']=>$value['newvalue']), "uid = '$value[uid]'");
					if($plus_group && empty($updated_members[$value['uid']])) {
						$member = DB::fetch_first('SELECT * FROM '.DB::table('common_member')." WHERE uid='$value[uid]'");
						if($member['credits'] < $plus_group['creditshigher']) {
							DB::update('common_member', array('credits'=>$plus_group['creditshigher'], 'groupid'=>$plus_group['groupid']), array('uid'=>$value['uid']));
						}
						$updated_members[$value['uid']] = true;
					}
					if($plus_credit) {
						DB::query('UPDATE '.DB::table('common_member_count')." SET $plus_credit[extcredits]=$plus_credit[extcredits]+$plus_credit[credits] WHERE uid='$value[uid]'");
					}
				}
				DB::delete('common_member_security', 'securityid IN ('.dimplode($_POST[ids]).')');
				cpmsg('members_verify_pass_success', 'action=members&operation=verify&page='.$page, 'succeed');
			} elseif($_POST['do'] == 'ignore') {
				DB::delete('common_member_security', 'securityid IN ('.dimplode($_POST[ids]).')');
				cpmsg('members_verify_ignore_success', 'action=members&operation=verify&page='.$page, 'succeed');
			}
		}
	}

	shownav('user', 'nav_members_verify');
	showsubmenu('nav_members_verify');

	$verifyitem = array();
	if($_GET['securityid']) {
		$_GET['securityid'] = intval($_GET['securityid']);
		$verifyitem = DB::fetch_first('SELECT * FROM '.DB::table('common_member_security')." WHERE securityid = '$_GET[securityid]'");
	}


	$searchlang = array();
	$keys = array('search', 'likesupport', 'resultsort', 'defaultsort', 'orderdesc', 'orderasc', 'perpage_10', 'perpage_20', 'perpage_50', 'perpage_100',
	'members_verify_dateline', 'members_verify_uid', 'action=members_verify_username', 'members_verify_fieldid');
	foreach ($keys as $key) {
		$searchlang[$key] = cplang($key);
	}
	loadcache('profilesetting');
	$fieldid_sel = '<select name="fieldid">';
	$fieldid_sel .= '<option value="">'.cplang('members_verify_fieldid_sel').'</option>';
	foreach($_G['cache']['profilesetting'] as $key=>$value) {
		if($value['needverify']) {
			$selected = ($key == $_GET['fieldid'] ? ' selected="selected"' : '');
			$fieldid_sel .= "<option value=\"$key\"$selected>$value[title]</option>";
		}
	}
	$fieldid_sel .= '</select>';
...
echo <<<SEARCHFORM
	<form method="get" autocomplete="off" action="$adminscript">
		<div class="block style4">
			...
			<tr><th>$searchlang[members_verify_uid]</th><td><input type="text" name="uid" value="$_GET[uid]"></td>
				<th>$searchlang[members_verify_username] * </th><td><input type="text" name="username" value="$_GET[username]">*$searchlang[likesupport]</td>
			</tr>
			<tr>
				<th>$searchlang[members_verify_fieldid]</th>
				<td colspan="3">$fieldid_sel</select>
				</td>
			</tr>
			<tr><th>$searchlang[members_verify_dateline]</th><td colspan="3">
			...
			<tr>
				...
				<td colspan="3">
					<select name="orderby">
					<option value="dateline"$orderby[dateline]>$searchlang[members_verify_dateline]</option>
					</select>
					<select name="ordersc">
...
SEARCHFORM;
	...
	showformheader('members&operation=verify');
	showtableheader('members_verify_securitylist');
	showsubtitle(array('', 'members_verify_username', 'members_verify_fieldid', 'members_verify_oldvalue', 'members_verify_newvalue', 'members_verify_dateline'));

	$count = DB::result(DB::query("SELECT COUNT(*) FROM ".DB::table('common_member_security')." WHERE $wheresql"), 0);
	if($count) {
		$query = DB::query('SELECT * FROM '.DB::table('common_member_security')." WHERE $wheresql $ordersql LIMIT $start, $perpage");
		while ($value = DB::fetch($query)) {
			showtablerow('', array('class="td25"', 'class=""', 'class="td28"'), array(
				"<input type=\"checkbox\" class=\"checkbox\" name=\"ids[]\" value=\"$value[securityid]\">",
				"<a href=\"home.php?mod=space&do=profile&uid=$value[uid]\" target=\"_blank\">$value[username]</a>",
				$_G['cache']['profilesetting'][$value['fieldid']]['title'],
				$value['oldvalue'],
				$value['newvalue'],
	...
	$radio_do = '<input type="radio" id="member_verify_do_pass" name="do" value="pass" /><label for="member_verify_do_pass">'.cplang('members_verify_pass').'</label>&nbsp;';
	$query = DB::query('SELECT groupid, grouptitle FROM '.DB::table('common_usergroup')." WHERE type='member'");
	$groupsel = '<select name="groupid">';
	while($value=DB::fetch($query)) {
		$groupsel .= "<option value=\"$value[groupid]\">$value[grouptitle]</option>";
	}
	$groupsel .= '</select>';
	$radio_do .= '<input type="checkbox" id="member_verify_do_pass_with_group" name="plus[]" value="group" /><label for="member_verify_do_pass_with_group">'.cplang('members_verify_pass_with_group').'</label>&nbsp;'.$groupsel;
	$creditsel = '<select name="extcredits">';
	foreach($_G['setting']['extcredits'] as $id=>$value) {
		$creditsel .= "<option value=\"$id\">$value[title]</option>";
	}
	$creditsel .= '</select>';
	$creditsel .= '<input type="text" name="credits" value="0" size="4" maxlength="6" />';
	$radio_do .= '<input type="checkbox" id="member_verify_do_pass_with_credit" name="plus[]" value="credit" /><label for="member_verify_do_pass_with_credit">'.cplang('members_verify_pass_with_credit').'</label>&nbsp;'.$creditsel;
	$radio_do .= '&nbsp;&nbsp;';
	$radio_do .= '<input type="radio" id="member_verify_do_ignore" name="do" value="pass" /><label for="member_verify_do_ignore">'.cplang('members_verify_ignore').'</label>&nbsp;';
	showsubmit('', '', '', '<input type="checkbox" name="chkall" id="chkall" class="checkbox" onclick="checkAll(\'prefix\', this.form, \'ids\')" /><label for="chkall">'.cplang('select_all').'</label>&nbsp;&nbsp;'.$radio_do.'<input type="submit" class="btn" name="securitysubmit" value="'.cplang('members_verify_submit').'" />', $multipage);
	showtablefooter();
	showformfooter();
...
function searchmembers($condition, $limit=5000, $start=0) {
	...
	if($option==false) {
		return $ms->cachesearch($condition, $start, $limit, true);
	} else {
		return $ms->cachesearch($condition, $start, $limit);
	}
}
...
function notifymembers($operation, $variable) {
	...
	if(!empty($_G['gp_current'])) {
		...
		if($operation == 'reward') {
			...
			if(!empty($medals)) {
				...
				$query = DB::query("SELECT uid, medals FROM ".DB::table('common_member_field_forum')." WHERE uid IN (".implode(',', $uids).")");
	...
	if($_G['gp_notifymember'] && in_array($_G['gp_notifymembers'], array('pm', 'email'))) {
		...
		$query = DB::query("SELECT uid, username, groupid, email FROM ".DB::table('common_member')." m WHERE $conditions LIMIT $current, $pertask");
		while($member = DB::fetch($query)) {
			$_G['gp_notifymembers'] == 'pm' ? notification_add($member['uid'], 'system', 'notice', array('subject' => $subject, 'message' => $message), 1) : sendmail("$member[username] <$member[email]>", $subject, $message);
			$continue = TRUE;
		}
	}
