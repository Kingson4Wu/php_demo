 ...
 *
 *      $Id: admincp_attach.php 6894 2010-03-26 09:34:23Z wangjinbo $
 */
...
if(!submitcheck('deletesubmit')) {
	...
	if(submitcheck('searchsubmit')) {
		...
		if($_G['gp_inforum'] != 'all') {
			$inforum = intval($_G['gp_inforum']);
		}
...
EOT;
		...
		echo $attachments;
		showsubmit('deletesubmit', 'submit', 'del', '<a href="###" onclick="$(\'admin\').style.display=\'none\';$(\'search\').style.display=\'\';" class="act lightlink normal">'.cplang('research').'</a>', $multipage);
		showtablefooter();
		showformfooter();
