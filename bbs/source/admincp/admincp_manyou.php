 ...
 *
 *      $Id: admincp_manyou.php 10439 2010-05-11 08:04:50Z zhengqingpeng $
 */
...
if($_G['gp_anchor'] == 'base') {
	...
	showsetting('setting_manyou_base_status', 'settingnew[my_app_status]', $setting['my_app_status'], 'radio', '', 1);
	...
	$haveinvitecode = !empty($setting['my_search_invite']) ? 'true' : 'false';
	...
	<ul onmouseover="altStyle(this);">
		<li$actives[1]><input type="radio" onclick="checkInviteCode(this.value);" $checkarr[1] value="1" name="settingnew[my_search_status]" class="radio">&nbsp;$lang[yes]</li>
		<li$actives[0]><input type="radio" onclick="$('hidden_setting_manyou_search_status').style.display = 'none';" $checkarr[0] value="0" name="settingnew[my_search_status]" class="radio">&nbsp;$lang[no]</li>
	</ul>
EOF;
	showsetting('setting_manyou_search_status', 'settingnew[my_search_status]', $setting['my_search_status'], $str, '', 1);
