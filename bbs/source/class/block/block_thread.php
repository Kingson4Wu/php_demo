 ...
 *
 *      $Id: block_thread.php 10773 2010-05-14 10:53:42Z xupeng $
 */
...
class block_thread {
	...
	function getdata($style, $parameter) {
		...
		if($recommend) {
			$sqlfrom .= " LEFT JOIN `".DB::table('forum_forumrecommend')."` fc ON fc.tid=t.tid";
		}
		...
		while($data = DB::fetch($query)) {
			...
			$list[] = array(
				...
				'fields' => array(
					...
					'avatar' => avatar($data['authorid'], 'small', true),
					'avatar_big' => avatar($data['authorid'], 'middle', true),
					'posts' => $data['posts'],
