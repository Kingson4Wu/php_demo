...
class block_announcement {
	...
	function getdata($style, $parameter) {
		...
		$sql = 'SELECT * FROM '.DB::table('forum_announcement')." WHERE starttime <= '$time' AND (endtime = '' || endtime >= '$time') $typesql $bannedids ORDER BY displayorder DESC LIMIT $startrow, $items";
		$query = DB::query($sql);
		while($data = DB::fetch($query)) {
			$list[] = array(
				...
				'url' => $data['type']=='1' ? $data['summary'] : 'forum.php?mod=announcement&id='.$data['id'],
				'pic' => '',
				'picflag' => '',
