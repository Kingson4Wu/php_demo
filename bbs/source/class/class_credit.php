 ...
 *
 *      $Id: class_credit.php 10662 2010-05-13 08:54:54Z monkey $
 */
...
class credit {
		...
		if($this->checklowerlimit && $uid && $_G['setting']['creditspolicy']['lowerlimit']) {
			...
			for($i = 1; $i <= 8; $i++) {
				if($_G['setting']['extcredits'][$i]) {
					...
					if($limit && $extcredit < 0 && ($member['extcredits'.$i] + $extcredit) < $limit) {
						return $i;
					}
				}
			}
	...
	function updatecreditbyrule($rule, $uids = 0, $coef = 1, $fid = 0) {
		...
		if($updatecredit || $this->extrasql) {
			$this->updatemembercount($creditarr, $uids, is_array($uids) ? false : true);
		}
	}

	function updatemembercount($creditarr, $uids = 0, $checkgroup = true) {
		global $_G;
		...
		if($uids && ($creditarr || $this->extrasql)) {
			...
			$sql = array();
			$allowkey = array('extcredits1', 'extcredits2', 'extcredits3', 'extcredits4', 'extcredits5', 'extcredits6', 'extcredits7', 'extcredits8', 'friends', 'posts', 'threads', 'digestposts', 'doings', 'blogs', 'albums', 'sharings', 'attachsize', 'views');
			$creditnotice = $_G['setting']['creditnotice'] && $_G['uid'] && $uids == array($_G['uid']);
			if($creditnotice && !isset($_G['cookiecredits'])) {
			...
			if($creditnotice) {
				dsetcookie('creditnotice', implode('D', $_G['cookiecredits']).'D'.$_G['uid'], 43200);
				dsetcookie('creditbase', '0D'.implode('D', $_G['cookiecreditsbase']), 43200);
			}
			if($sql) {
	...
	function checkusergroup($uid) {
		...
		if($uid) {
			...
			if(is_array($member) && empty($group) || $group['type'] == 'member' && !($member['credits'] >= $group['creditshigher'] && $member['credits'] < $group['creditslower'])) {
				...
				if(DB::num_rows($query)) {
					...
					if($member['groupid'] != $newgroupid) {
						...
						$groupid = $newgroupid;
						if($self) {
							$_G['member']['credits'] = $groupid = $newgroupid;
						}
	...
	function updatecheating($rulelog, $needle, $newcycle) {
		if($needle) {
			...
			if($rulelog['isnew']) {
				...
				DB::update('common_credit_rule_log_field', $logarr, array('uid'=>$rulelog['uid'], 'clid'=>$rulelog['uid']));
			}
		}
	}
