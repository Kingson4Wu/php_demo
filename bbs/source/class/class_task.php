 ...
 *
 *      $Id: class_task.php 9108 2010-04-27 02:42:04Z monkey $
 */
...
class task {
	...
	function tasklist($item) {
		...
		$query = DB::query("SELECT t.*, mt.csc, mt.dateline FROM ".DB::table('common_task')." t
			...
			WHERE $sql AND t.available='2' ORDER BY displayorder, taskid DESC");
	...
	function view($id) {
		...
		if(!$this->task = DB::fetch_first("SELECT t.*, mt.status, mt.csc, mt.dateline, mt.dateline AS applytime FROM ".DB::table('common_task')." t LEFT JOIN ".DB::table('common_mytask')." mt ON mt.uid='$_G[uid]' AND mt.taskid=t.taskid WHERE t.taskid='$id' AND t.available='2'")) {
			showmessage('undefined_action');
		}
		...
		if($this->task['status'] == '-1') {
			if($this->task['period']) {
				$allowapply = TIMESTAMP - $this->task['dateline'] >= $this->task['period'] * 3600 ? 3 : -7;
				$this->task['t'] = tasktimeformat($this->task['period'] * 3600 - TIMESTAMP + $this->task['dateline']);
			} else {
				$allowapply = -4;
			}
	...
	function apply($id) {
		...
		if($this->task['relatedtaskid'] && !DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_mytask')." WHERE uid='$_G[uid]' AND taskid='".$this->task['relatedtaskid']."' AND status='1'")) {
			...
			$mytask = DB::fetch_first("SELECT mt.*, t.period, t.periodtype FROM ".DB::table('common_mytask')." mt
				INNER JOIN ".DB::table('common_task')." t ON t.taskid=mt.taskid
	...
	function reward_magic($magicid, $num) {
		...
		if(DB::result_first("SELECT COUNT(*) FROM ".DB::table('common_member_magic')." WHERE magicid='$magicid' AND uid='$_G[uid]'")) {
