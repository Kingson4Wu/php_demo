 ...
 *
 *      $Id: class_core.php 10981 2010-05-19 03:44:00Z zhaoxiongfei $
 */
...
class discuz_core {
	...
	function _init_config() {
		...
		$_config['security']['authkey'] = empty($_config['security']['authkey']) ? md5($_config['cookie']['cookiepre'].$_config['db'][1]['dbname']) : ($_config['security']['authkey']);
		...
		define('STATICURL', !empty($this->config['output']['staticurl']) ? $this->config['output']['staticurl'] : 'static/');
	}
	...
	function _init_output() {
		...
		if($this->config['output']['gzip'] && !$this->var['gp_inajax'] && EXT_OBGZIP) {
	...
	function _init_user() {
		...
		if($this->init_user) {
			...
			if($user && $user['groupexpiry'] > 0 && $user['groupexpiry'] < TIMESTAMP && getgpc('mod') != 'groupexpiry' && CURSCRIPT != 'member') {
	...
	function _init_misc() {
		...
		if($this->init_user) {
			if($this->var['group'] && isset($this->var['group']['allowvisit']) && !$this->var['group']['allowvisit']) {
				...
				} elseif(CURSCRIPT != 'member' && CURSCRIPT != 'misc') {
		...
		if($this->var['setting']['bbclosed'] && CURSCRIPT != 'admin' && !(in_array($this->var['mod'], array('logging', 'seccode')) || getglobal('adminid', 'member') == 1)) {
	...
	function _init_setting() {
		...
		if(!empty($this->var['cookie']['styleid']) && $this->var['cookie']['styleid'] != $this->var['setting']['styleid'] && is_array($this->var['setting']['styles']) && array_key_exists($this->var['cookie']['styleid'], $this->var['setting']['styles'])) {
			loadcache('style_'.$this->var['cookie']['styleid']);
			if($this->var['cache']['style_'.$this->var['cookie']['styleid']]) {
				$this->var['style'] = $this->var['cache']['style_'.$this->var['cookie']['styleid']];
			}
