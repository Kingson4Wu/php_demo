/*
	...
	$Id: portal_diy.js 10933 2010-05-18 07:44:13Z zhangguosheng $
*/
	var drag = new Drag();
	drag.extend({
		'getBlocksTimer' : '',
		...
		openBlockEdit : function (e,tab) {
			...
			this.removeMenu();
			showWindow('showblock', 'portal.php?mod=portalcp&ac=block&op=edit&bid='+bid+'&tab='+tab+'&tpl='+document.diyform.template.value, 'get', -1);
		},
		...
		openStyleEdit : function (e) {
			...
			var h = '<h3 class="flb"><em>背景和边框的属性</em><span><a href="javascript:;" class="flbc" onclick="drag.closeStyleEdit(\''+id+'\');return false;" title="关闭">\n\
				关闭</a></span></h3>';
		...
		clearAll : function () {
			...
			for (var i in this.data) {
				for (var j in this.data[i]) {
					if (this.data[i][j].name.indexOf('_temp')<0) {
		...
		getFrameHtml : function (type) {
			...
			var len = cols.length;
			if (len == 1) {
				str += '<div id="'+id+'_left" class="'+this.moveableColumn+' col-c"></div>';
			} else if (len == 2) {
				var cls1 = cols[0] == '1' ? ' sd' : ' mn';
				var cls2 = cols[1] == '1' ? ' sd' : ' mn';
				if (cols[0] == '1' && cols[1] == '1') {
					cls1 = ' col-l';
					cls2 = ' col-r';
				}
				str += '<div id="'+id+'_left" class="'+this.moveableColumn + cls1 + '"></div>';
				str += '<div id="'+id+'_center" class="'+this.moveableColumn + cls2 + '"></div>';
			} else if (len == 3) {
				str += '<div id="'+id+'_left" class="'+this.moveableColumn+' col-l"></div>';
				str += '<div id="'+id+'_center" class="'+this.moveableColumn+' col-c"></div>';
				str += '<div id="'+id+'_right" class="'+this.moveableColumn+' col-r"></div>';
			}
			str += '</div>';
			return str;
		},
	...
	spaceDiy.extend({
		...
		disablePreviewButton : function () {
			if ($('preview')) {
				...
				$('savecachemsg').style.display = 'none';
			}
		},
		cancelDIY : function () {
			this.disablePreviewButton();
