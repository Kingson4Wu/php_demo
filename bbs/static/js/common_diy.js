/*
	...
	$Id: common_diy.js 10813 2010-05-17 03:19:19Z zhangguosheng $
*/
var TC=0;
...
styleCss.prototype.addRule = function(selector, styles, n, porperty) {
	...
	if (i != -1) {
		...
		if (s) {
			s = s.replace(selector,'').replace('{', '').replace('}', '');
	...
	if (this.sheet.insertRule) {
		...
		this.sheet.addRule(selector, s, n);
	}
};
...
var Util = {
	...
	formatColor : function (color) {
		return color == '' || color.indexOf('#')>-1 ? color : '#'+Util.rgb2hex(color);
	}
};
...
(function(){
	...
	Drag.prototype = {
		getTmpBoxElement : function () {
			if  (!this.tmpBoxElement) {
				...
				this.tmpBoxElement.style.width = this.overObj.offsetWidth-2+"px";
				this.tmpBoxElement.style.height = this.overObj.offsetHeight-2+"px";
		...
		initFrame : function (frameEle) {
			...
			if ($(frameId+'_edit')) $(frameId+'_edit').onclick = function (e){Drag.prototype.toggleMenu.call(_method, e, this);};
				...
				if (Util.hasClass(columns[i], this.moveableColumn)) {
					...
					for (var j = 0; j < eleLen; j++) {
						...
						if (Util.hasClass(ele, this.frameClass) || Util.hasClass(ele, this.tabClass)) {
							...
							var _method = this;
							if ($(ele.id + '_edit')) $(ele.id + '_edit').onclick = function(e){Drag.prototype.toggleMenu.call(_method, e, this);};
		...
		showEdit : function (e) {
			...
			if (targetObject) {
				...
				if (typeof targetFrame == 'object') Util.show(targetFrame.id + '_edit');
			}
		},
		hideEdit : function (e) {
			e = Util.event(e);
			var targetObject = Util.getTarget(e,'className',this.moveableObject);
			var targetFrame = Util.getTarget(e,'className',this.frameClass);
			if (typeof targetFrame == 'object') Util.hide(targetFrame.id + '_edit');
			if (typeof targetObject == 'object') Util.hide(targetObject.id + '_edit');
		},
		toggleMenu : function (e, obj) {
			e = Util.event(e);
			e.stopPropagation();
			var name = obj.id.replace('_edit', '');
		...
		drag : function (e) {
			e = Util.event(e);
			if(!this.isDragging) {
				this.dragObj.style.filter = "alpha(opacity=30)" ;
				this.dragObj.style.opacity = 0.3 ;
		...
		doEndDrag : function () {
			...
			if (Util.hasClass(this.dragObj, this.frameClass)) {
				var flag = Util.hasClass(this.dragObj.parentNode, this.areaClass) ? true : false;
				this._replaceFrameColumn(this.dragObj, flag);
			}
		},
		_replaceFrameColumn : function (ele,flag) {
			var children = ele.childNodes;
			if (Util.hasClass(ele, 'frame-2-1') || Util.hasClass(ele, 'frame-1-2') || Util.hasClass(ele, 'frame-3-1') || Util.hasClass(ele, 'frame-1-3')) {
				for (var i in children) {
					if (Util.hasClass(children[i], this.moveableColumn)) {
						if (flag) {
							children[i].className = children[i].className.replace(' col-l', ' mn');
							children[i].className = children[i].className.replace(' col-r', ' sd');
						} else {
							children[i].className = children[i].className.replace(' mn', ' col-l');
							children[i].className = children[i].className.replace(' sd', ' col-r');
						}
					}
				}
			}
		...
		_getMoveableArea : function (ele) {
			...
			for (var i = 0; i < len; i++) {
				...
				if (children[i].nodeName.toUpperCase() == 'DIV' && Util.hasClass(children[i],this.areaClass)) {
					this.moveableArea.push(children[i]['id']);
				} else {
	...
	DIY.prototype = {
		...
		getSpacecssStr : function() {
			...
			for (var i in this.spacecss) {
				...
				for (var j in rule) {
					...
					if (typeof content == "function" || content == '') continue;
					one += j + ":" + content + ";";
				}
				if (one == '') continue;
				css += i + " {" + one + "}";
		...
		changeLayout : function (newLayout) {
			...
			drag.initPosition();
			if (!drag.isChange) {drag.isChange = true;drag.setClose();}
		},
		...
		hideBg : function () {
			var random = Math.random();
			this.setStyle(this.currentDiy, 'background-image', 'none', random);
		...
		setStyle : function (currentDiv, property, value, random, num){
			...
			value = value == '' ? '' : value.indexOf('!important') > -1 ? value : value + ' !important';
			this.styleSheet.addRule(selector,property+':'+value,num,property);
			Util.recolored();
