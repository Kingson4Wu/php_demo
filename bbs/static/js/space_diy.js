/*
	...
	$Id: space_diy.js 10688 2010-05-13 11:32:49Z monkey $
*/
	var drag = new Drag();
	drag.extend({

		setDefalutMenu : function () {
			this.addMenu('default', '…æ≥˝', 'drag.removeBlock(event)');
			this.addMenu('block', ' Ù–‘', 'drag.openBlockEdit(event)');
		},
			...
			if (el != null) {
				...
				x.get('home.php?mod=spacecp&ac=index&op=getblock&blockname='+blockname+'&inajax=1',function(s) {
					if (s) {
						...
						el.id = blockname;
						s = s.replace(/\<script.*\<\/script\>/ig,'<font color="red"> [javascriptΩ≈±æ±£¥Ê∫Ûœ‘ æ] </font>');
						el.innerHTML = s;
